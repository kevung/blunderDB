...
.. csv-table::
   ...
   "ph:race", "La position est dans une phase de jeu donnée : ``opening`` (ouverture), ``middlegame`` (milieu de partie), ``race`` (course) ou ``bearoff`` (sortie des pions). Répétable (``ph:race ph:bearoff``). L'étiquette est calculée à partir du plateau, jamais modifiable ; la commande ``blunderdb repair`` la recalcule."
   ...
   "co:user", "La position porte un commentaire d'une provenance donnée : ``user`` (écrit par vous), ``xg``, ``gnubg``, ``bgf`` (apporté par l'import d'un match) ou ``unknown``. Répétable (``co:xg co:gnubg``)."
