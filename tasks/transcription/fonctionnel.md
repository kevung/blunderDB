...
Décisions de référence : [ADR-0043](../../docs/adr/0043-transcribing-a-match-is-not-playing-one.md)
(transcrire n'est pas jouer) et [ADR-0044](../../docs/adr/0044-a-transcription-is-a-draft-that-owns-its-match.md)
...
| `beaver` | oui, si argent | faux | posé sur les positions, jamais une Action (ADR-0043) |
...
`kind`. Le `side` est **possédé** par l'Action (ADR-0044 §4) : proposé à la saisie, il ne
...
1. Le document est rejoué en entier ; s'il contient des Incohérences, l'utilisateur en est
   averti et peut enregistrer quand même (ADR-0043 : rien n'est refusé).
...
3. `ingest.WriteMatch` en **mode remplacement** : dans une transaction, les `game`/`move` du
   ...
   par `positionIsHeldSQL` ; **rien ne passe par le Trash** (ADR-0044 §3). Premier
