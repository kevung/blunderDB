...
| Plateau | position du Cursor via `positionStore` ; flèches via `selectedMoveStore` ; lot 2 : `quizPlayStore` réarmé par le panneau (le réducteur `quizPlay.js` est déjà branché dans `boardInteractions.js:318-328`) ; le jet saisi, un glissé hors des coups légaux rend le coup libre (`dragStep`), enregistré par Entrée avec `BoardAfter` (ADR-0052) | `Board.svelte`, `boardInteractions.js` |
