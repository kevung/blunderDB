...
| D | Tenant purge | [13-tenant-purge.md](13-tenant-purge.md) | 2-3 d | medium | **done** — `postgres.Storage.PurgeTenant` deletes every tenant-scoped table in one transaction, exposed as `POST /v1/tenant.purge` (postgres only, `CodeInvalid` 400 on SQLite), with a `purgeOrder`/`rlsTables` parity guard |
