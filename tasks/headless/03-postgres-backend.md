...
| 3 | **Done.** Matches + games + moves (`matchStore`). Full `MatchStore` interface: header CRUD, `CreateGame`/`Games`, `CreateMove`/`Moves`, `MovePositions` (the move-position join), `SwapPlayers`, `MergePlayers`, `SetLastVisitedPosition`/`LastVisited`, `DeleteCascade`. `DeleteCascade` runs inside the caller's `Tx`; the match delete cascades to game/move/move_analysis via `ON DELETE CASCADE`, then orphaned positions (no move, no collection ref) are dropped. `SwapPlayers`/`MergePlayers` open their own transaction via a `txBeginner` type-assertion so they are atomic whether bound to the pool or a tx. Postgres-specific test `matches_postgres_test.go` (`//go:build postgres`) covers every method; the shared contract `Match/*` cases stay pending until P2 PR3 lands the SQLite match store. |
