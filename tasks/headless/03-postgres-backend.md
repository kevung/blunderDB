...
| 3 | **Done.** Matches + games + moves (`matchStore`). Full `MatchStore` interface: header CRUD, `CreateGame`/`Games`, `CreateMove`/`Moves`, `MovePositions` (the move-position join), `SwapPlayers`, `MergePlayers`, `SetLastVisitedPosition`/`LastVisited`, `DeleteCascade`. `DeleteCascade` cascades the match delete to game/move/move_analysis via `ON DELETE CASCADE`, then drops orphaned positions (no move, no collection ref). `DeleteCascade`/`SwapPlayers`/`MergePlayers` open their own transaction via a `txBeginner` type-assertion so they are atomic whether bound to the pool or a tx. Postgres-specific test `matches_postgres_test.go` (`//go:build postgres`) covers every method. (P2 PR3 later applied the D2 amendment — `DeleteCascade` dropped its `tx Tx` parameter — and enabled the shared contract `Match/*` cases, which now run green on PostgreSQL too.) |
| 4 | **Done.** Tournaments (`tournamentStore`) + collections + collection_position (`collectionStore`). Full `TournamentStore` (CRUD, match membership, `SetMatchByName` find-or-create, `ReorderMatches`, `TournamentOf`) and `CollectionStore` (CRUD, `Reorder`, `AddPosition(s)`/`RemovePosition(s)`, `ReorderPositions`, `MovePosition`/`CopyPosition`, `Positions`, `CollectionsOf`, `PositionIndexMap`). Multi-statement ops go through a shared `withTx` helper (savepoint-capable). The shared contract `Tournament/AddRemoveMatch` case is enabled — green on both backends. Collections get a postgres-specific test (`collections_postgres_test.go`); the contract `Collection/*` case stays pending until P2 PR4 lands the SQLite collection store. |
| 5 | **Done.** Comments (`commentStore`) + anki/FSRS (`ankiStore`: `anki_deck` + `anki_card`). Full `CommentStore` (Add/Update/Delete, `DeleteForPosition`, `Text` blank-line concatenation, `ByPosition`/`ListAll`/`Search` — `ILIKE` for the case-insensitive text search) and `AnkiStore` (deck CRUD, `ResetDeck`, `Sync`/`SyncWithPositions`, `DeckPositions`, `DeckStats`, `NextCard`/`ReviewCard` driving the go-fsrs scheduler). Timestamp columns are native `TIMESTAMPTZ`: scanned into `time.Time` and formatted via `tsTime`; due-count cutoffs use `now()` server-side instead of a bound parameter. `SyncWithPositions` dedups via `ON CONFLICT (deck_id, position_id) DO NOTHING` inside a `withTx`. The shared contract `Anki/ReviewUpdatesScheduling` case is enabled — green on both backends. Postgres-specific tests (`comments_postgres_test.go`, `anki_postgres_test.go`) cover what the contract omits (comment CRUD / concatenation / search, anki deck CRUD / collection sync / missing-deck + missing-card errors). |
| 6 | **Done (filters + command_history + search_history + metadata).** All three list-bearing tables are tenant-scoped (`tenant_id = tenantID(scope)`), `INSERT … RETURNING id`, trim via `id NOT IN (… ORDER BY timestamp DESC, id DESC LIMIT n)`. `metadataStore`: `Version`/`SetVersion`/`Load`/`Save` over the global `metadata` table (`ON CONFLICT (key) DO UPDATE`), and a tenant-scoped `Counts` (`COUNT(*) … WHERE tenant_id = $1` across position/analysis/match/game/move). Shared contract cases `Filter/SaveAndList`, `History/SaveLoadClear`, `SearchHistory/SaveListDelete`, `Metadata/Counts` enabled — green on both backends. **`session` deferred:** SQLite stores session state as `session_*` rows in the **global** `metadata` table; a correct multi-tenant Postgres session store needs the tenant-scoped metadata keys that **P4** introduces. The `Session/SaveLoadEmpty` contract case stays skipped. |
| 7 | **metadata done in PR6.** **stats deferred:** the `StatsStore` is a stub on *both* backends — the statistical SQL was never ported to the Storage layer (it still lives in `database/db_stats.go`, used by the GUI/CLI via the `Database` wrapper). Porting it is its own chantier, not Postgres-specific; the `Stats/AggregateCounts` contract case stays skipped on both. |
| 8 | **Tenant-isolation test done** (`tenant_isolation_postgres_test.go`): the same position under two tenants yields distinct rows (UNIQUE `(tenant_id, zobrist_hash)`), cross-tenant `Load` fails, filter names are unique per-tenant not globally, and `Counts` are per-tenant. **Remaining (optional):** the RLS opt-in migration (`002_*.sql`, currently reserved for P4) and `concurrent_writes_test.go`. |
