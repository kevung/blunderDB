...
- [x] Créer `db_stats.go` avec les structs :
  ```go
  ...
  ```
- [x] Structs auxiliaires `TournamentStats{ID, Name, Date, PR, NumDecisions}`, `MatchStats{ID, Date, PlayerName, PR, NumDecisions}`, `CubeActionStats{Action, PR, NumDecisions, BlunderCount}`, `ErrorBucket{MinMP, MaxMP, Count}`, `BlunderEntry{PositionID, MatchID, TournamentID, ErrorMP, Description}`.
- [x] MWC fields (`MWCGlobal`, `MWCChecker`, etc.) : ajoutés en fiche 02, **pas ici**.

### 2. Extraire la constante `errExpr`

- [x] Déclarer dans `db_stats.go` :
  ```go
  const statsErrExpr = "CASE WHEN p.decision_type = 1 THEN a.cube_error ELSE a.best_move_equity_error END"
  ```
- [x] Remplacer l'usage inline à `db_search.go:361` par cette constante (refactor pur).

### 3. Helper `buildStatsWhereClause`

- [x] Fonction `buildStatsWhereClause(filter StatsFilter) (whereSQL string, args []any)` — **privée**, partagée avec la fiche 03.
- [x] Jointures impliquées : `position p JOIN analysis a ON a.position_id = p.id JOIN move mv ON mv.position_id = p.id JOIN game g ON g.id = mv.game_id JOIN match m ON m.id = g.match_id LEFT JOIN tournament t ON t.id = m.tournament_id`.
- [ ] Clauses :
...
- [x] Tests unitaires : 1 test par clause, 1 test combinaison complète (vérifie `whereSQL` + ordre `args`).
...
- [x] **Règle d'agrégation** : chaque PR calculé comme `500 × SUM(err) / 1000 / COUNT(*)` (pondéré). Le PR par tournoi est calculé directement sur les positions du tournoi, **pas dérivé** du PR par match.
- [x] Helper Go `pr(sumErrMP int64, nDecisions int) float64 { return 500 * float64(sumErrMP) / 1000 / float64(nDecisions) }`.

### 5. Tests unitaires

- [x] `db_stats_test.go` dans la racine.
- [x] **Test anti-moyenne-des-moyennes** : fixture avec 2 tournois. Tournoi A = match1 (100 décisions, total err 1000) + match2 (10 décisions, total err 200). PR match1 = 5, PR match2 = 10. Moyenne des PR = 7.5. PR pondéré = 500×1200/1000/110 = 5.45. **Vérifier que `ComputeStats` retourne 5.45, pas 7.5.**
- [x] Test filtre par joueur : fixture avec 2 matchs, joueur « Alice » en player1 dans un match et player2 dans l'autre. Vérifier que filtrer par « Alice » compte bien les décisions de Alice des deux matchs (jointure `move.player` correcte).
- [x] Test filtre combiné (player + tournament + date range).
- [x] Test top blunders ordonnés décroissants.
- [x] Test histogramme : somme des counts = total décisions.
- [x] Test performance (benchmark) : `BenchmarkComputeStats` sur une fixture de 10 000+ positions. Budget : < 200 ms.

### 6. Exposer à Wails

- [x] Ajouter `Bind(db)` dans `main.go` **ne touche rien** — `*Database` est déjà bound, `ComputeStats` sera auto-exposé dans `wailsjs/go/main/Database.d.ts` après `wails dev`.
- [x] Aucune modif de `main.go` dans cette fiche. La fiche 04 consomme le binding.

## Acceptance criteria

- [x] `go test -run TestComputeStats ./...` vert (tous les tests listés ci-dessus passent).
- [x] `go test -run TestBuildStatsWhereClause ./...` vert.
- [x] `go test -bench BenchmarkComputeStats -benchtime 5s ./...` < 200 ms sur 10 000 positions (~90 ms observé).
- [x] `go vet ./...` clean.
- [x] Pas de régression : `go test ./...` reste vert.
- [x] Pas de modif de schéma, pas de migration.
