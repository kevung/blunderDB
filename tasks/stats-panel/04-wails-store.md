...
- [x] Lancer `wails dev` après la fiche 03.
- [x] Vérifier que `frontend/wailsjs/go/main/Database.d.ts` contient bien :
  ```ts
  ...
  ```
- [x] Si un type n'apparaît pas (ex. `StatsFilter` non exporté par `wails`), ajouter un tag struct ou un renommage pour le rendre visible.

### 2. Créer `statsStore.js`

- [x] `frontend/src/stores/statsStore.js` :
  ```javascript
  ...
  ```
- [x] Auto-fetch : exposer une fonction `async function refreshStats()` qui appelle `ComputeStats(filter)`, met à jour `statsResultStore`. Appelée manuellement depuis `StatsPanel` au mount et sur changement de filtre (pas via `derived` pour contrôler le timing).

### 3. Créer `positionLoader.js`

- [x] `frontend/src/services/positionLoader.js` :
  ```javascript
  import {
  ...
  ```
- [x] Identifier le store de résultats de recherche (probablement dans `frontend/src/stores/searchStore.js` ou équivalent). Documenter l'API réutilisée dans un commentaire Go-doc-style au-dessus de `loadPositionsFromSelection`.
- [x] `openTournamentInPanel` / `openMatchInPanel` : localiser le store de sélection dans `tournamentStore.js` / matchPanel-related store ; écrire une simple setter.

### 4. Tests Vitest

- [x] `frontend/src/services/positionLoader.test.js` :
...
- [x] `frontend/src/stores/statsStore.test.js` :
  - Mock `ComputeStats`.
...
- [x] Ajouter à `frontend/src/stores/uiStore.js` :
  ```javascript
  export const PANEL = {
      // … existing …
      STATS: 'stats',
  ...
  ```
- [x] Ajouter à `frontend/src/commandProcessor.js` :
  ```javascript
  else if (command === 'stats' || command === 'st') {
      callbacks.onToggleStats?.();
  }
  ```
- [x] Dans `frontend/src/App.svelte` (fonction `initCommandProcessor`) : `onToggleStats: () => togglePanel(PANEL.STATS)`.
- [x] Pas de mount du composant `StatsPanel` dans cette fiche — squelette en fiche 05.

## Acceptance criteria

- [x] `wails dev` démarre sans warning. `frontend/wailsjs/go/main/Database.d.ts` contient les 4 nouvelles méthodes.
- [x] `npm test` vert (tests Vitest ci-dessus passent).
- [x] `npm run lint` clean sur les nouveaux fichiers.
- [x] La commande `:stats` dans la ligne de commande bascule bien l'état de `openPanels` (vérifiable manuellement via le devtool).
