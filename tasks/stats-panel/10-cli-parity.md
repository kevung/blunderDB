...
- [x] Ajouter des flags à la sous-commande existante :
...
- [x] Construire `StatsFilter` à partir des flags, appeler `db.ComputeStats(filter)`.

### 2. Format texte

- [x] Sections ordonnées :
...
- [x] Utiliser `tabwriter.NewWriter` pour l'alignement propre.
- [x] Afficher `—` (em-dash) pour les valeurs MWC indisponibles (money-game).

### 3. Format JSON

- [x] Marshaller `StatsResult` directement via `encoding/json.MarshalIndent`.
- [x] Prérequis : tags JSON sur `StatsResult` et ses sous-structs (fiche 01 peut les ajouter si pas déjà fait).

### 4. Tests

- [x] `cli_stats_test.go` (nouveau ou étendu) :
...
- [x] Test de régression : l'ancienne invocation sans flags (`list --type stats`) produit toujours une sortie utile (défauts raisonnables).

### 5. Documentation CLI

- [x] Mettre à jour `CLI_USAGE.md` avec la nouvelle section `stats` détaillée :
...
- [x] `./blunderdb list --db testdata/Quiz.db --type stats` produit un rapport texte lisible.
- [x] `./blunderdb list --db testdata/Quiz.db --type stats --metric mwc` bascule en MWC.
- [x] `./blunderdb list --db testdata/Quiz.db --type stats --format json` produit un JSON valide.
- [x] Les valeurs CLI correspondent bit-à-bit à celles du Dashboard GUI pour le même filtre (test manuel ou diff scripté).
- [x] `go test -run TestCLIStats ./...` vert.
- [x] `go test ./...` reste vert.
