...
- [x] Ajouter à `db_met.go` :
  ...
  ```
- [x] Exportée (majuscule) car consommée depuis `db_stats.go` et potentiellement utile ailleurs.

### 2. Tests unitaires

- [x] Créer `db_met_mwc_test.go` (ou étendre `db_met_test.go` s'il existe).
- [x] **Test round-trip** : pour une set de scores variés (DMP 1pt, Crawford, 3-away 5-away, 7-pt, 11-pt, 21-pt) et plusieurs valeurs de cube (1, 2, 4) :
...
- [x] **Test money-game** : `matchLength = 0` → retour `NaN` (utiliser `math.IsNaN`).
- [x] **Test cube dégénéré** : DMP (1-away/1-away, cube=1) → `mwcWin=1.0, mwcLose=0.0`, `denom=1.0`, ΔMWC = ΔEMG × 0.5. Valeur testée explicitement.
- [x] **Test Crawford** : score 4-away / Crawford (1pt trailer), cube=1. Vérifier que la valeur retournée reste plausible (non NaN, signe cohérent).
...
- [x] Ajouter les champs MWC dans `StatsResult` (fiche 01) :
  ```go
  ...
  ```
- [x] La MWC **agrégée** d'un tournoi/match/breakdown est la **somme** des MWC loss par décision (cumul), **pas** `(mwcWin − mwcLose) / 2 × PR_moyen` — ce serait faux car `(mwcWin − mwcLose)` varie par position.

### 4. Adapter `ComputeStats`

- [x] Les requêtes 3–7 de la fiche 01 doivent désormais ramener aussi les colonnes nécessaires à la conversion MWC par ligne : `p.score_1, p.score_2, mv.player as fMove, p.cube_value, p.match_length`.
- [x] Boucle Go post-SQL (approche 1 du plan §Architecture backend) :
...
- [x] Alternative perf (si benchmark le justifie) : changer ces requêtes en `SELECT per-row (err, score_1, score_2, mv.player, cube_value, match_length, tournament_id, match_id, best_cube_action)` sans GROUP BY, puis agréger en Go (PR + MWC simultanément). **Recommandé** car simplifie le code et ne dégrade pas la perf sur les bases usuelles.
- [x] Positions money-game (`match_length = 0`) : leur contribution MWC est `NaN` ; en Go, les exclure de la somme MWC mais pas de la somme PR. Flagger chaque agrégat avec un bool `MWCAvailable bool` (true si au moins une position du groupe n'est pas money-game).

### 5. Tests d'intégration

- [x] Étendre `db_stats_test.go` de la fiche 01 :
...
- [x] `go test -run TestConvertEMGLossToMWCLoss ./...` vert, incluant le test round-trip sur ≥ 5 scores différents.
- [x] `go test -run TestComputeStats ./...` vert (tests MWC inclus).
- [x] Pas de modif de schéma, pas de migration.
- [x] `go vet ./...` clean.
- [x] Aucune utilisation de CGO ou de nouvelle dépendance.
