...
### F0 — Le probe devient la mesure de référence [S]

- [ ] `cost_probe_test.go` : ajouter les colonnes *remplissage moyen des lots* (positions
      évaluées / créneaux offerts) et *noyau* (`go`, `avx2`, `neon`) ; les compteurs
      correspondants sur `Searcher` à côté de `evals`/`pruneEvals`/`cacheHits`.
- [ ] Ajouter une ligne « 2-ply canonique, `WithWorkers(NumCPU)` ».
- [ ] Ajouter un `BenchmarkEvaluateBatch` (à côté de `BenchmarkEvaluate`,
      `position_test.go:111`) qui mesure le coût *par position* à largeur pleine.
- [ ] Ajouter un `BenchmarkDecision2Ply` sous `testing.Short()` pour que `-bench` donne le
      chiffre sans variable d'environnement.
- [ ] Consigner les chiffres de départ **dans ce fichier**, section 1, avec la date et le
      commit — le probe n'affirme rien, le plan si.

### F1 — Le noyau batché [L]

Le cœur du plan. Tout ce qui suit est bit-identique par construction, et le test le prouve.

- [ ] **API** : `Evaluator.EvaluateBatch(features [][]float32 ou [B][196]float32, probs
      *[B][NumOutputs]float32)`, largeur `B` fixe (constante du package, 8 au départ ; le
      déroulage interne peut traiter 2 ou 4 groupes de 8, à mesurer). Un lot partiel est
      **complété par duplication** de la dernière position, jamais par des zéros : l'appelant
      ne raisonne pas sur la fin du lot, et une voie dupliquée est vérifiable (`==` avec sa
      jumelle).
- [ ] **Layout** : activations feature-major `act[j*B+n]` comme le C
      (`gn_infer_reference.c:305-398`) ; poids inchangés (row-major `[out][in]`, format
      `BGNN`). Chaque sortie `i` : `acc[0..B) = bias[i]`, puis pour `j` croissant `acc[n] +=
      w[i][j] * act[j*B+n]`. C'est l'ordre du scalaire actuel, voie par voie.
- [ ] **Sparsité de la couche 1** : à la transposition, collecter l'union des indices `j`
      non nuls sur le lot et ne parcourir que ceux-là. Exact : `acc + w × 0.0 = acc` en IEEE
      (à vérifier au bit près sur `reference.bin`, y compris le cas `acc = -0`). Note du C :
      indexer `w[nonzero[idx]]` directement était *plus lent* que la boucle dense ; compacter
      les poids de la couche 1 dans un tampon contigu par lot, comme lui
      (`gn_infer_reference.c:342-360`).
- [ ] **Noyau AVX2** via avo : `vbroadcastss` du poids, `vmulps` puis `vaddps` séparés
      (**jamais** `vfmadd*`), un registre de 8 voies par groupe ; tuile sur plusieurs sorties
      `i` pour réutiliser la colonne chargée (4 sorties × 8 voies = 4 accumulateurs est le
      point de départ à mesurer). `VZEROUPPER` en sortie. ReLU par `vmaxps` avec zéro ;
      sigmoïde de la dernière couche en Go (5 sorties, `math.Exp` en float64 comme
      aujourd'hui, `network.go:267-269`).
- [ ] **Noyau NEON** via avo (arm64) : `fmul` + `fadd` séparés (jamais `fmla`), 4 voies par
      registre, donc 2 registres par groupe de 8.
- [ ] **Repli pur Go** de même layout, même ordre de sommation, `float32(a*b)` explicite
      pour interdire la contraction (comme `network.go:246`). C'est la référence du test
      bit-à-bit.
- [ ] **Sélecteur** : `BLUNDERDB_GAMMONNET_KERNEL=go|avx2|neon`, lu une fois au chargement ;
      défaut = meilleur disponible (`cpu.X86.HasAVX2`, arm64 → NEON toujours présent). Une
      valeur demandée mais indisponible est une erreur au chargement, pas un repli silencieux.
- [ ] **Dénormalisés** : Go ne positionne pas FTZ/DAZ (MXCSR) et ne met pas FZ dans FPCR sur
      AArch64 ; les trois chemins suivent donc l'IEEE. Le prouver, pas le supposer : un
      vecteur de `reference.bin` à activations minuscules, ou un vecteur construit, dans le
      test bit-à-bit.
- [ ] **Test bit-à-bit** (`kernel_identity_test.go`) : les 2 000 vecteurs de
      `testdata/reference.bin` et ≥ 2 000 positions issues d'une recherche 1-ply réelle,
      passés dans le noyau assembleur et dans le repli Go de la même machine, `==` sur
      chaque bit des 5 sorties **et** des activations intermédiaires. Sur les trois runners
      CI (AVX2 vs Go sous Linux et Windows, NEON vs Go sous macOS ; la transitivité couvre
      AVX2 vs NEON). Lots pleins, partiels, et de taille 1.
- [ ] `parity_test.go` (1e-6 contre le C) et `gold_test.go` passent sans retouche — sinon D2
      est violée, on s'arrête.
- [ ] **Cible mesurée** (F0) : ≤ 60 µs par position à largeur pleine (le C fait 41 µs avec
      gcc à largeur 32), soit ×7 sur `BenchmarkEvaluate`.
- [ ] `go vet`, `golangci-lint` (les fichiers `.s` générés portent un en-tête « generated by
      avo, do not edit » et le générateur vit sous `internal/` ou `_asm/` avec un `go:generate`).

### F2 — La recherche alimente le noyau [M]

- [ ] `shallowFill` (`search.go:374-397`) : retirer les hits cache, encoder les survivants
      dans le tampon feature-major du lot, appeler `EvaluateBatch`, redistribuer. Le petit
      réseau d'élagage passe par le même chemin (il n'a toujours pas accès au cache,
      `cache.go:20-22`).
- [ ] Le tampon de lot vit dans le `Searcher` (pas d'allocation par nœud, comme aujourd'hui
      pour `a`/`b`).
- [ ] Le probe (F0) donne le remplissage : si < 70 % à 2-ply canonique, ouvrir la fiche
      « regroupement des 21 lancers » (D4) ; sinon la fermer avec le chiffre.
- [ ] `TestParallelSearchIsBitIdentical` et `gold_test.go` inchangés et verts.
- [ ] **Cible mesurée** : décision 2-ply canonique série < 1 s (D1).

### F3 — Le lot analyse en parallèle [M]
...
### F4 — Le panneau en direct utilise tous les cœurs [M]
