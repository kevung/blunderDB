...
// « Des positions comme celle-ci » (ADR-0043), côté base.
...
//
// A separate call from LoadPositionsByFilters because the DISTANCE is part of
// the answer, and it belongs to the comparison, not to the Position. A bare
// `like` is resolved to a target id by the caller; nothing here guesses it.
...
// far. Only ids cross the Wails bridge; the distance travels with the id since
// it cannot be recomputed without the target.
