...
// This file holds the legacy, hand-written SQL implementation of the stats
// queries. It is NOT production code: the Database methods delegate to the
// storage backend (see db_stats.go). These legacy* functions and their SQL
// helpers survive only as the differential parity oracle that the stats tests
// pin the storage backend against (stats_storage_parity_test.go,
// db_stats_drilldown_test.go). Keeping them in a _test.go file keeps the
// duplicate SQL out of the shipped binary while preserving the parity net.
