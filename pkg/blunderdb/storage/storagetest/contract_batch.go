...
// beyond "the same rows as the unit reads": the order the caller asked for
// (positions), silence about ids that name nothing (all of them), oldest first
// within a position (comments), and id order (moves).

func testPositionLoadMany(t *testing.T, s storage.Storage) {
	...
	got, err := s.Positions().LoadMany(ctx, "", []int64{ids[2], 999999, ids[0]})
	if err != nil {
		t.Fatalf("LoadMany: %v", err)
	}
	if len(got) != 2 || got[0].ID != ids[2] || got[1].ID != ids[0] {
		t.Fatalf("LoadMany order/skip: got %d positions, ids %v", len(got), positionIDs(got))
	}
	if got[0].Score != [2]int{3, 0} {
		t.Errorf("LoadMany reconstructs the position: score = %v, want [3 0]", got[0].Score)
	}

	if got, err := s.Positions().LoadMany(ctx, "", nil); err != nil || len(got) != 0 {
		t.Fatalf("LoadMany(nil) = %v, %v; want empty, nil", got, err)
	}
}

func itoa(n int64) string { return strconv.FormatInt(n, 10) }

func positionIDs(ps []*domain.Position) []int64 {
