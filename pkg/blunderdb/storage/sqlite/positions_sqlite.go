...
// positionCols is the column list read back into a Position; the order matches
// engine.ReconstructPosition's parameters.
...
func (s *positionStore) Save(ctx context.Context, scope string, p *domain.Position) (int64, error) {
	...
	res, err := s.db.ExecContext(ctx, positionInsertSQL,
		...
		engine.EncodeBoardCompact(norm.Board))
	if err != nil {
	...
	if affected, _ := res.RowsAffected(); affected > 0 {
		...
		// Hash already present: return the existing row's id.
