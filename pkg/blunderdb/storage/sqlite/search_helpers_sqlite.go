...
// This file holds the search-filter helpers ported from the Database wrapper's
// db_search.go and db_filter_match.go. The pure parsers are copied verbatim;
// the predicates that need database access are re-expressed against execer
// (package database imports storage/sqlite, so the reverse import is not
// possible).
...
func loadCommentText(ctx context.Context, db execer, positionID int64) (string, error) {
	var text string
	err := db.QueryRowContext(ctx, `SELECT text FROM comment WHERE position_id = ?`, positionID).Scan(&text)
