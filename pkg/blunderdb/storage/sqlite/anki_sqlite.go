...
func (s *ankiStore) randomCard(ctx context.Context, deckID, excludePositionID int64) (domain.AnkiCard, error) {
	query := `SELECT ` + ankiCardCols + ` FROM anki_card WHERE deck_id = ?`
	args := []any{deckID}
	...
	c, err := scanAnkiCard(s.db.QueryRowContext(ctx, query, args...))
	if errors.Is(err, sql.ErrNoRows) {
		return domain.AnkiCard{}, storage.ErrNotFound
	}
	if err != nil {
...
func (s *ankiStore) RandomCard(ctx context.Context, scope string, deckID, excludePositionID int64) (*domain.AnkiReviewCard, error) {
	card, err := s.randomCard(ctx, deckID, excludePositionID)
	if errors.Is(err, storage.ErrNotFound) && excludePositionID != 0 {
		...
		card, err = s.randomCard(ctx, deckID, 0)
	}
	if err != nil {
		if errors.Is(err, storage.ErrNotFound) {
			return nil, err
		}
		return nil, fmt.Errorf("sqlite: random anki card of deck %d: %w", deckID, err)
	}
	pos, err := s.loadPosition(ctx, card.PositionID)
	if err != nil {
		return nil, fmt.Errorf("sqlite: random anki card of deck %d: %w", deckID, err)
