...
// Save appends an executed search to the scope's history and trims it to the
...
func (s *SearchHistoryStore) Save(ctx context.Context, scope string, command, position string) error {
	...
	if _, err := s.DB.Exec(ctx,
		`INSERT INTO search_history (`+col+`, command, position, timestamp) VALUES (?,?,?,?)`,
		arg, command, position, time.Now().UnixMilli()); err != nil {
