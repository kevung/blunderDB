...
// inclusive everywhere, as in the SQLite backend.
...
// counts both players' moves (see Compute). Mirrors the SQLite copy.
...
func buildBaseWhereClause(tenant int64, filter storage.StatsFilter) (whereSQL string, args []any) {
	return buildBaseWhereClauseSeat(tenant, filter, true)
}
...
func buildBaseWhereClauseSeat(tenant int64, filter storage.StatsFilter, seatAware bool) (whereSQL string, args []any) {
...
func (s *statsStore) Compute(ctx context.Context, scope string, filter storage.StatsFilter) (*storage.StatsResult, error) {
	...
	// ── Snowie ER (global) ────────────────────────────────────────────────────
	// One player's errors over BOTH players' checker moves (gnuBG
	// formatgs.c:415-424), so numerator and denominator need different WHERE
	// clauses — see the SQLite copy for the full rationale.
	{
		snowieFilter := filter
		snowieFilter.DecisionType = -1
		numWhere, numArgs := buildBaseWhereClause(tenant, snowieFilter)
		var snowieSumErr int64
		_ = s.db.QueryRow(ctx, rebind(
			`SELECT COALESCE(SUM(`+statsErrExpr+`),0)::BIGINT `+
				statsBaseJoin+numWhere),
			numArgs...,
		).Scan(&snowieSumErr)

		denWhere, denArgs := buildBaseWhereClauseSeat(tenant, snowieFilter, false)
		var snowieCheckerCnt int
		_ = s.db.QueryRow(ctx, rebind(
			`SELECT COALESCE(SUM(CASE WHEN p.decision_type=0 THEN 1 ELSE 0 END),0)::BIGINT `+
...
// why the badge is the reference player's own PR rather than a both-players pool.
...
// design (player selection and decision type). Mirrors the SQLite copy.
...
// buildMatchWhereClause renders the filter's match-level predicates against the
// match table alone, scoped to the tenant, for the queries that count matches
// and games rather than decisions.
func buildMatchWhereClause(tenant int64, filter storage.StatsFilter) (whereSQL string, args []any) {
	clauses := []string{"m.tenant_id = ?"}
	args = append(args, tenant)
...
// Mirrors the SQLite backend; the arithmetic is shared in
// storage.BuildPlayerRows.
func (s *statsStore) PlayerTable(ctx context.Context, scope string, filter storage.StatsFilter) ([]storage.PlayerRow, error) {
	...
	statsWhere, statsArgs := buildStatsWhereClause(tenant, f)
	baseWhere, baseArgs := buildBaseWhereClause(tenant, f)
	matchWhere, matchArgs := buildMatchWhereClause(tenant, f)
	...
	rows, err := s.db.Query(ctx, rebind(
		`SELECT `+moverNameExpr+` AS pname, p.decision_type,`+
			` COALESCE(SUM(`+statsErrExpr+`),0)::BIGINT, COUNT(*)::BIGINT,`+
			` COALESCE(SUM(CASE WHEN (`+statsErrExpr+`) > 0 THEN 1 ELSE 0 END),0)::BIGINT,`+
			` COALESCE(SUM(CASE WHEN (`+statsErrExpr+`) >= ? THEN 1 ELSE 0 END),0)::BIGINT `+
	...
	rows, err = s.db.Query(ctx, rebind(
		`SELECT `+moverNameExpr+` AS pname, COALESCE(SUM(`+statsErrExpr+`),0)::BIGINT `+
	...
	rows, err = s.db.Query(ctx, rebind(
		`SELECT `+moverNameExpr+` AS pname, COALESCE(SUM(mv.luck_mp),0)::BIGINT, COUNT(mv.luck_mp)::BIGINT
		 FROM move mv
		 JOIN game g ON g.id = mv.game_id
	...
	rows, err = s.db.Query(ctx, rebind(
		`SELECT COALESCE(m.player1_name,''), COALESCE(m.player2_name,''), COALESCE(m.match_length,0)::BIGINT,
		        COALESCE((SELECT SUM(g.points_won) FROM game g
		                  WHERE g.match_id = m.id AND g.winner = 1), 0)::BIGINT,
		        COALESCE((SELECT SUM(g.points_won) FROM game g
		                  WHERE g.match_id = m.id AND g.winner = -1), 0)::BIGINT,
		        COALESCE((SELECT COUNT(*) FROM move mv2
		                  JOIN position p2 ON p2.id = mv2.position_id
		                  JOIN game g2 ON g2.id = mv2.game_id
		                  WHERE g2.match_id = m.id AND p2.decision_type = 0), 0)::BIGINT
