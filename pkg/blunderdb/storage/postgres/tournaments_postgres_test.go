...
// the paths the shared contract suite does not exercise.
func TestTournamentCRUD(t *testing.T) {
	...
	if got.CreatedAt == "" || got.UpdatedAt == "" {
		t.Errorf("Get timestamps empty: %+v", got)
	}
...
// TestTournamentReorderAndDelete checks ReorderMatches order and that deleting
// a tournament unlinks rather than deletes its matches.
func TestTournamentReorderAndDelete(t *testing.T) {
	ctx := context.Background()
	s, _ := openMatchStore(t)
	...
	if _, err := s.Matches().Get(ctx, "", id1); err != nil {
	...
	if _, err := s.Tournaments().TournamentOf(ctx, "", id1); !errors.Is(err, storage.ErrNotFound) {
		t.Errorf("TournamentOf after tournament delete: got %v, want ErrNotFound", err)
	}
}
