...
//
// Beyond the HOME BOARD the polynomial describes positions it was never
// fitted on (ADR-0027 §9). Against a TS-07-06 oracle (20 000 positions per
// sample: six/six, seven/six, seven/seven) it improves p99 and the maximum in
// all three, so it applies unchanged, with BeyondHomeP99/Max as the bound
// measured there. TestCorrection_BeyondTheHomeBoard re-measures it.
...
//
// The boards are as wide as the loaded table: the convolution reads only two
// roll distributions, so width costs it nothing; the six-point-fitted
// correction is what width puts in question.
