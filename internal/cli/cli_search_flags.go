...
func defineSearchFlags(fs *flag.FlagSet) *searchFlags {
	return &searchFlags{
		...
		like:              fs.Int("like", 0, "Rank this position's neighbours, nearest first: same kind of decision, same regime for a cube decision, met in another match (replaces the query rather than narrowing it)"),
