...
| [0024](0024-the-evaluator-batches-positions-one-per-lane-and-keeps-the-scalar-sum-order.md) | The evaluator batches positions, one per lane, and keeps the scalar sum order | proposed | 2026-09-02 | executes the bounded follow-up 0011 left open | The network kernel vectorises over positions (one per SIMD lane, no FMA, ascending-j float32 sums) so it stays bit-identical to the scalar port on AVX2, NEON and pure Go; parallelism becomes production behaviour; nothing changes what the engine plays. |
