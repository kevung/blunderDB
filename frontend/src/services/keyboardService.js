...
import {
    firstPosition, previousPosition, nextPosition, lastPosition,
    saveCurrentPosition, updatePosition, deletePosition,
    toggleAnalysisPanel, toggleCommentPanel, toggleMetadataModal,
    toggleAnkiPanel, toggleMatchPanel, toggleCollectionPanelAction,
    toggleTournamentPanel, toggleEPCMode, togglePipcount,
    loadAllPositions, loadRandomPosition,
...
export function handleKeyDown(event) {
    ...
    if (event.key === 'Escape') {
        ...
        if (!showComment) { event.preventDefault(); firstPosition(); }
    } else if (!event.ctrlKey && event.key === 'h') {
        if (!showComment) firstPosition();
    } else if (!event.ctrlKey && event.key === 'ArrowLeft') {
        if (!showComment && !get(selectedMoveStore)) { event.preventDefault(); previousPosition(); }
    } else if (!event.ctrlKey && event.key === 'k') {
        if (!showComment && !get(selectedMoveStore)) previousPosition();
    } else if (!event.ctrlKey && event.key === 'ArrowRight') {
        if (!showComment && !get(selectedMoveStore)) { event.preventDefault(); nextPosition(); }
    } else if (!event.ctrlKey && event.key === 'j') {
        if (!showComment && !get(selectedMoveStore)) nextPosition();
    } else if (!event.ctrlKey && event.key === 'PageDown') {
        if (!showComment) { event.preventDefault(); lastPosition(); }
