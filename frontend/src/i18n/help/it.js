...
<p>Due impostazioni governano l'ordinamento delle <strong>posizioni vicine</strong> (token <code>like</code>, vedere Pannello Ricerca): il numero di vicine restituite e la distanza massima oltre la quale una posizione cessa di esserlo. Questa distanza vale zero per impostazione predefinita, cioè nessun limite: la scala dipende dalla fase di gioco, e un valore scelto qui si leggerebbe come una misura. Il token <code>like&lt;12</code> impone la propria per una ricerca, senza toccare l'impostazione.</p>
...
<h3>Il cestino</h3>
<p>Eliminare una posizione, una collezione o un commento passa da un <strong>cestino</strong>: l'eliminazione avviene davvero, ma una copia di ciò che sparisce è conservata trenta giorni. Il comando <code>trash</code> apre la finestra che le elenca, ciascuna con <em>Ripristina</em> ed <em>Elimina</em>.</p>
...
<p>Correggere è digitare sulla cella in cui ci si trova. Con il cursore su un'azione, una cifra ne ricomincia il lancio sul posto, e anche le quattro azioni di cubo valgono come correzioni: su un rifiuto, <em>t</em> — o il pulsante <strong>Accetta</strong> — scrive un'accettazione <strong>al posto</strong> del rifiuto, senza doverlo cancellare e poi inserire. La partita riprende allora il suo corso: si apre una cella subito dopo l'accettazione, dalla parte di chi ha raddoppiato, e il seguito della partita vi si digita come al solito, inserito davanti all'apertura della partita successiva, finché la partita non termina. Gli stessi tasti riempiono la cella che un inserimento ha appena aperto: <em>i</em> e poi <em>d</em> inserisce un raddoppio davanti all'azione mirata.</p>
...
<p>Reinserire i due dadi nella cella di <strong>apertura</strong> di una partita decide di nuovo chi comincia: il dado del giocatore 1 si digita per primo, quello del giocatore 2 dopo, e vince il più alto — il dado grande per primo, il turno va al giocatore 1, in basso sulla tavola; il piccolo per primo, va al giocatore 2, in alto. La convalida è immediata al secondo dado, come per un'apertura nuova. <strong>La prima mossa di pedine della partita segue il nuovo vincitore</strong> e cambia colonna con lui: il suo campo non era stato scelto, era proposto dall'apertura — il vincitore del lancio gioca entrambi i dadi senza reinserirli. È la sola azione che la correzione di un'apertura sposta: il resto della partita conserva i suoi campi, e una mossa già data all'altro giocatore con <em>s</em> non viene ripresa.</p>
...
<p>Una proposta si conferma con <strong>un clic</strong> su <em>Lancia</em>. <strong>Lancia tutto</strong> conferma l'intera coda in due clic, dopo averne mostrato l'elenco; <em>Conferma</em> è in testa a quell'elenco. «Ignora per ora» non scrive nulla: il motore è deterministico, e la proposta torna identica alla chiamata successiva. <em>Abbina a mano</em> resta sempre disponibile — il motore propone, il direttore decide.</p>
<p>Un match abbinato a mano senza numero di tavolo prende il primo tavolo libero. Se la sala è piena, viene avviato comunque e la sua casella compare in fondo alla griglia, «senza tavolo», finché non lo si sposta su un tavolo.</p>
...
<p>La scheda <strong>Classifica</strong> mostra la classifica corrente, sezione per sezione, con i premi quando è impostato un montepremi. Due pari merito condividono il posto e il premio. <strong>Chiudi il torneo</strong> congela la classifica finale. La classifica si copia in CSV, nella lingua dell'interfaccia, o si salva in un file: <strong>Salva…</strong> apre la finestra di dialogo del sistema su un nome proposto, il torneo seguito dalla parola «classifica» e dalla data del giorno, e il file contiene esattamente il CSV copiato. Da riga di comando, <code>blunderdb tournament standings</code> scrive lo stesso CSV.</p>
<p>Chiudere senza match in corso è un clic. Con match in corso, la Classifica dice quanti sono e attende un secondo clic sul posto: chiudere congela la classifica senza di loro, e il loro risultato non si inserisce più. <strong>Riapri</strong> si conferma allo stesso modo; la classifica finale smette allora di essere finale, e la riapertura resta nel registro.</p>
...
<p>Qui si impostano: le fasi e la lunghezza delle loro partite, le lunghezze turno per turno di un tabellone («15, 13, 11» si legge dall'ultimo turno all'indietro), il numero di tavoli, il ritmo previsto in minuti per punto (8 per impostazione predefinita; la barra dell'orologio e la fine stimata partono da esso), le pause della giornata, il montepremi (quota d'iscrizione, trattenuta del club, scala per sezione) e la cartella del display.</p>
...
<p>Le impostazioni restano accessibili <strong>durante il torneo</strong>: abbassare la soglia alle 22 per finire prima, aggiungere una consolazione il sabato sera, finché il sorteggio del tabellone non è fatto. Ciò che è allora congelato appare in grigio con la sua ragione: il formato di una fase aperta, il numero di vite che ha distribuito e, dal sorteggio di una fase, la sua consolazione, la riconciliazione, la ricarica, la dimensione dei gironi e il numero di qualificati. Salvare durante il torneo mostra prima l'elenco di ciò che cambierà, e chiede conferma.</p>
...
<ul>
<li>l'<strong>esercizio</strong> — <em>Scores</em>, <em>Pions</em> (conteggio), <em>Bearoff</em>, <em>Évaluation</em> o <em>Décision</em>;</li>
...
<p><em>Scores</em> e <em>Pions</em> si <strong>dichiarano</strong>: calcolate a mente, cliccate « Révéler », e la verità appare. Ogni numero è allora <strong>giusto per difetto</strong> — cliccate quello che avete sbagliato per segnarlo come <strong>fallo</strong> (<em>Tab</em> poi <em>Spazio</em> fa lo stesso gesto da tastiera), e un secondo clic annulla il segno. Non si scrive nulla: un conteggio di pedine o una casella di tabella è giusta o sbagliata, e scriverla non insegna nulla più che leggerla.</p>
...
<p>Finché una domanda del pannello Allenamento è posta sulla tavola, i tasti che scorrono la lista non la fanno scorrere: la domanda tiene la tavola.</p>
...
<tr>
<td>1 … 6 (cursore sull'apertura di una partita)</td>
<td>Reinserire il lancio d'apertura: dado del giocatore 1 e poi del giocatore 2, registrati al secondo dado. Vince il più alto — il dado grande per primo, il turno va al giocatore 1 in basso; il piccolo per primo, al giocatore 2 in alto. La prima mossa di pedine della partita segue il nuovo vincitore.</td>
</tr>
<tr>
<td>GIÙ, j</td>
