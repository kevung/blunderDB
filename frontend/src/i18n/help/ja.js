...
export default {
    manual: `
                    ...
                    <p>
                        評価パネルを開くには、<strong>Ctrl+E</strong> を押すか、下部パネルの評価タブをクリックするか、コマンドラインに <strong>epc</strong> と入力します。ボードは標準的な
                        ベアオフ構成（15 チェッカー）で初期化されます。
                    </p>
                    <p>
                        マウスを使ってホームボードのポイント上のチェッカーを自由に追加または削除できます。EPC 値は専用の評価パネルにリアルタイムで表示され、各プレイヤーについて次を表示します:
                    </p>
                    <ul>
                    ...
                    <p>評価パネルを閉じるには、もう一度 <strong>Ctrl+E</strong> を押すか、別のタブに切り替えます。</p>
                    <p>
                        純粋なベアオフの局面では、レース表に両プレイヤーの勝率も表示されます。局面がtwo-sidedデータベースの範囲内なら(内蔵はプレイヤーごとに6枚まで、設定のベアオフタブから11枚までの拡張版をダウンロード可能)、正確なマネー・エクイティ(最善でない各決定の下に最善との差)と最善のキューブ判断が表示されます。範囲外では勝率は推定値(誤差付きの「推定」バッジ)となり、判断は表示されません。手番はプレイヤーのベアオフ/スコア枠をクリック、キューブの位置は盤上のキューブをクリックして編集します。
                    </p>
                    <p><strong>チャレンジ</strong>にチェックすると編集のたびに結果が隠され、ゾーンをクリックすると表示されます。EPCとキューブ判断を自分で見積もってから確認する練習に最適です。</p>
                    ...
                    <p>
                        <strong>セッションの制限：</strong> デッキ設定で、1 回のセッションの枚数を制限できます。上限に達するとその旨を伝えてセッションが終わり、自由練習は予定に影響を与えずに続けられます。<em>0</em> は 1 枚も出題しない設定であり、「制限なし」とは別物です。
                    </p>
                    <p>
                        <strong>保持率：</strong> 目標保持率は、負荷と質のバランスについてのあなたの選択です。設定画面にはその横に、実際の復習から<em>測定した</em>保持率が表示されます。参考情報であり、制御ではありません。目標の変更は遡及しません。各カードは次の復習から新しいペースになります。
                    </p>
                    <p>
                        <strong>解答の表示：</strong> カードは問いを出します。考えたうえで <strong>スペース</strong> キーを押す（またはマスクされた領域をクリックする）と、その局面に保存された解析が表示されます。評価ボタンのすぐ下に現れ、ボタンは手の届く位置に留まります。評価するために解答を表示する必要はありません。次のカードで再びマスクされますが、タブを切り替えただけでは戻りません。
                    </p>
                    <p>
                    ...
                    <p>
                        エクスポート時（<strong>export_db</strong>、またはエクスポートダイアログ）には、互いに独立した2つの保護を自由に有効にできます。片方だけ、もう片方だけ、または両方同時に:
                    </p>
                    <ul>
                        <li>
                            <strong>透かし：</strong>エクスポートしたファイルに出所（誰が作成したか、任意のメモ）を記載します。透かしはあなたの発行者の識別情報で署名されており、他人の名前を騙って改変や偽造をすることはできません
                            — ただし削除は防げず、コピーも一切禁止しません。
                        </li>
                        <li>
                            <strong>パスワード：</strong>エクスポートを暗号化された <strong>.dbx</strong> コンテナに格納します。データベース自体ではなく、受け渡し中のファイルを保護します。パスワードを渡した相手は開くことができ、
                            出所はパスワードなしでも読み取れます。
                        </li>
                    </ul>
                    <p>
                        あなたの発行者の識別情報（透かしに署名する鍵）は、出所を記載した最初のエクスポート時に自動的に作成されます。設定の<strong>発行者の識別情報</strong>タブから確認、エクスポート、再生成できます。
                    </p>
`,
    shortcuts: `
                    <h3>データベース</h3>
