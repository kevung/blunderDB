...
<p>Deux réglages gouvernent le classement des <strong>positions voisines</strong> (jeton <code>like</code>, voir Panneau Recherche) : le nombre de voisines rendues, et la distance maximale au-delà de laquelle une position cesse d'en être une. Cette distance vaut zéro par défaut, c'est-à-dire aucun plafond : l'échelle dépend de la phase de la partie, et une valeur choisie ici se lirait comme une mesure. Le jeton <code>like&lt;12</code> impose la sienne pour une recherche, sans toucher au réglage.</p>
...
<h3>La corbeille</h3>
<p>Supprimer une position, une collection ou un commentaire passe par une <strong>corbeille</strong> : la suppression a bien lieu, mais une copie de ce qui disparaît est gardée trente jours. La commande <code>trash</code> ouvre la fenêtre qui les liste, avec pour chacune <em>Restaurer</em> et <em>Supprimer</em>.</p>
...
<p>La <strong>barre du brouillon</strong>, en tête du panneau, ne porte que les gestes qui font sortir le brouillon de lui-même, les deux flèches d'annulation et le sens du plateau.</p>
...
<p>Corriger, c'est taper sur la cellule où l'on est. Le curseur posé sur une action, un chiffre en recommence le jet sur place, et les quatre gestes de videau valent eux aussi comme corrections : sur une passe, <em>t</em> — ou le bouton <strong>Prendre</strong> — écrit une prise <strong>à la place</strong> de la passe, sans qu'il faille la supprimer puis insérer. La partie reprend alors son cours : une cellule s'ouvre juste après la prise, au camp du doubleur, et la suite de la partie s'y tape comme d'habitude, insérée devant l'ouverture de la partie suivante, jusqu'à ce que la partie se termine. Les mêmes touches remplissent la cellule qu'une insertion vient d'ouvrir : <em>i</em> puis <em>d</em> insère un double devant l'action visée.</p>
...
<p>Ressaisir les deux dés sur la cellule d'<strong>ouverture</strong> d'une partie décide à nouveau qui commence : le dé du joueur 1 se tape en premier, celui du joueur 2 ensuite, et c'est le plus fort qui l'emporte — gros dé d'abord, le trait revient au joueur 1, en bas du plateau ; petit dé d'abord, il revient au joueur 2, en haut. La validation est immédiate au second dé, comme pour une ouverture neuve. <strong>Le premier coup de pions de la partie suit le nouveau vainqueur</strong> et change de colonne avec lui : son camp n'avait pas été choisi, il était proposé par l'ouverture — le gagnant du jet joue les deux dés sans les ressaisir. C'est la seule action que corriger une ouverture déplace : la suite de la partie garde ses camps, et un coup déjà donné à l'autre joueur par <em>s</em> n'est pas repris.</p>
...
<p>C'est là que le directeur passe l'essentiel de son temps. Elle porte, de haut en bas : les <strong>avertissements</strong> du moteur, qui restent visibles et ne bloquent jamais rien ; le bouton <strong>Imprimer la feuille</strong> d'appariements ; la <strong>file des propositions</strong> ; la <strong>grille des tables</strong> ; la <strong>dernière décision</strong> ; et les joueurs libres.</p>
<p>Une proposition se confirme d'<strong>un clic</strong> sur <em>Lancer</em>. <strong>Tout lancer</strong> confirme la file entière en deux clics, après en avoir montré la liste. « Ignorer pour l'instant » n'écrit rien : le moteur est déterministe, et la proposition revient identique au prochain appel. <em>Apparier à la main</em> reste offert en permanence — le moteur propose, le directeur décide.</p>
<p>Un match apparié à la main sans numéro de table prend la première table libre. Si la salle est pleine, il est lancé quand même et sa case apparaît au bout de la grille, « sans table », jusqu'à ce qu'on le déplace sur une table.</p>
...
<p>Un retrait se fait <em>maintenant</em> ou <em>après son match en cours</em>, selon que le joueur part tout de suite ou finit ce qu'il joue.</p>
<h4>Arbres, emplacements, classement, historique</h4>
<p>L'onglet <strong>Arbres</strong> dessine les tableaux et, pour un suisse, le tableau des vies. Un match déjà joué y porte son résultat ; un match que le moteur signale y est marqué sur place.</p>
<p>L'onglet <strong>Emplacements</strong> relie le tournoi à la bibliothèque. Chaque match du tournoi est un emplacement, que l'on peut remplir de deux façons : transcrire le match sur-le-champ (Panneau Transcription), ou y rattacher un match déjà importé. <strong>Rien n'est rattaché par déduction</strong> : une coïncidence de noms est une suggestion à accepter, un appariement partiel n'est même pas suggéré, et si le fichier d'un match rattaché contredit le résultat enregistré, l'écart est montré sans être résolu — pendant un tournoi, la parole du directeur fait foi.</p>
<p>L'onglet <strong>Classement</strong> montre le classement courant, section par section, avec les prix lorsqu'une dotation est réglée. Deux ex æquo partagent la place et le prix. <strong>Clore le tournoi</strong> fige le classement final ; le classement s'exporte en CSV, dans la langue de l'interface.</p>
<p>L'onglet <strong>Historique</strong> est le journal en clair : une ligne par décision, dans l'ordre, filtrable par joueur ou par match. C'est ce qu'un directeur relit après une contestation, et c'est là qu'une décision ancienne se corrige ou s'annote.</p>
...
<p>Se règlent ici : les phases et leur longueur de match, les longueurs tour par tour d'un tableau (« 15, 13, 11 » se lit du dernier tour vers le premier), le nombre de tables, les pauses de la journée, la dotation (droit d'entrée, retenue du club, barème par section) et le dossier d'affichage.</p>
<p>Les réglages restent accessibles <strong>en cours de tournoi</strong> : baisser la bascule à 22 h pour finir plus tôt, ajouter une consolante le samedi soir. Deux choses seulement sont alors figées — le format d'une phase ouverte et le nombre de vies qu'elle a distribué — et elles sont grisées avec leur raison. Enregistrer en cours de tournoi montre d'abord la liste de ce qui va changer, et demande confirmation.</p>
...
<p>Hors de l'interface, la sous-commande <code>blunderdb tournament</code> relit un tournoi dirigé sans interface graphique : <code>list</code>, <code>verify</code>, <code>standings</code>, <code>page</code> et <code>export</code>. Voir Interface en ligne de commande (CLI).</p>
<h3>Panneau Stats</h3>
<h4>Introduction</h4>
...
<ul>
<li>l'<strong>exercice</strong> — <em>Scores</em>, <em>Pions</em>, <em>Bearoff</em>, <em>Évaluation</em> ou <em>Décision</em> ;</li>
...
<p>Tant qu'une question du panneau Entraînement est posée sur le plateau, les touches qui parcourent la liste ne la font pas défiler : la question garde le plateau.</p>
<h3>Affichage</h3>
<table>
<thead>
<tr>
...
<tr>
<td>GAUCHE / DROITE</td>
<td>Dans la fiche de résultat : le joueur de gauche gagne, celui de droite gagne.</td>
</tr>
<tr>
<td>CTRL-Z</td>
<td>Reprendre la dernière décision d'un tournoi dirigé.</td>
</tr>
<tr>
<td>ÉCHAP</td>
...
<tr>
<td>1 … 6 (curseur sur l'ouverture d'une partie)</td>
<td>Ressaisir le jet d'ouverture : dé du joueur 1 puis dé du joueur 2, validés au second dé. Le plus fort l'emporte — gros dé d'abord, le trait revient au joueur 1 en bas ; petit dé d'abord, au joueur 2 en haut. Le premier coup de pions de la partie suit le nouveau vainqueur.</td>
</tr>
<tr>
<td>BAS, j</td>
