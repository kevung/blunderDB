...
<ul>
<li>l'<strong>exercice</strong> — <em>Scores</em>, <em>Pions</em> ou <em>Bearoff</em> ;</li>
...
<p><code>train scores</code>, <code>train pips</code> et <code>train bearoff</code> ouvrent le panneau et démarrent directement ; <code>train tp</code> et <code>train takepoint</code> sont des synonymes de <code>train scores</code>, <code>train epc</code> de <code>train bearoff</code>.</p>
<h4>Les trois exercices</h4>
...
<p>Le chronomètre part à l'affichage de la question et s'arrête à « Révéler » ou « Valider » ; la question suivante se prépare pendant que vous répondez, elle n'est donc jamais chronométrée avec la vôtre. Cocher ses fautes n'est pas chronométré non plus. Avec une limite, une question restée sans réponse à l'échéance se révèle seule et compte <strong>hors délai</strong> : tous ses nombres sont faux, et son temps n'entre pas dans la médiane — on ne mesure pas une réponse qui n'a pas été donnée.</p>
...
<h3>Micro-entraînement : le quiz</h3>
<p>Un exercice se répond encore au clavier, dans une bande affichée au-dessus du plateau : <code>train quiz</code> lance une session de cinq questions. La question EST la position affichée — le plateau est celui de l'application, et la bande ne porte que la question, la saisie et la correction. <em>Entrée</em> vérifie puis passe à la suivante, <em>Échap</em> quitte la session.</p>
<p>Seul le résumé de la session est conservé, dans les métadonnées de la base : elle ne garde pas la trace question par question, et rien n'est écrit tant qu'elle n'est pas terminée. Quitter en cours de route n'enregistre donc rien.</p>
<p>Le panneau Anki fait mémoriser ; le quiz <strong>teste</strong>. Cinq positions déjà analysées sont tirées de la liste parcourue, et il faut décider :</p>
<ul>
...
<tr>
<td>train</td>
<td>Ouvre le panneau Entraînement. Avec un argument, ouvre et démarre : <code>train scores</code> (la fiche de score d'un score tiré au sort ; <code>train tp</code> et <code>train takepoint</code> sont des synonymes), <code>train pips</code> (le compte de pions des deux camps), <code>train bearoff</code> (l'EPC des deux camps sur une position engendrée ; <code>train epc</code> est un synonyme). <code>train quiz</code> lance le micro-entraînement de la bande.</td>
</tr>
<tr>
<td>tp2</td>
