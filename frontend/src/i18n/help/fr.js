...
<p>
    La calculatrice EPC (Effective Pip Count) calcule le pip count effectif des positions de sortie. Elle utilise la base de données de sortie unilatérale à 6 points de GNUbg pour obtenir des valeurs
    d'EPC exactes.
</p>
<p>
    Pour ouvrir le panneau Eval, appuyez sur <strong>Ctrl+E</strong>, cliquez sur l'onglet Eval du panneau inférieur ou tapez <strong>epc</strong> dans la ligne de commande. Le plateau est initialisé
    avec une configuration standard de sortie (15 pions).
</p>
<p>
    Vous pouvez librement ajouter ou retirer des pions sur les flèches du jan intérieur à l'aide de la souris. Les valeurs d'EPC s'affichent en temps réel dans le panneau Eval dédié, indiquant pour
    chaque joueur :
</p>
<ul>
...
<p>
    Sur une position de sortie pure, un tableau course affiche en outre les probabilités de gain des deux joueurs et, lorsque la position est couverte par une base two-sided (base intégrée jusqu'à 6
    pions par joueur, base étendue téléchargeable jusqu'à 11 pions via l'onglet Bearoff de la configuration), les équités money exactes — avec, sous chaque décision non optimale, l'écart d'équité à la
    meilleure décision — et la meilleure décision de videau. Hors de ce domaine, la probabilité de gain est estimée (badge « estimé » avec sa marge d'erreur) et aucune décision n'est affichée. Le
    joueur au trait s'édite en cliquant le rectangle sortie/score d'un joueur, la position du videau en cliquant le videau du plateau.
</p>
<p>
    La case <strong>Défi</strong> masque les résultats à chaque modification de la position ; cliquez une zone pour la révéler — idéal pour s'entraîner à estimer l'EPC et la décision de videau avant
    de vérifier.
</p>
...
<tr>
<td>import, i</td>
<td>Importe une ou plusieurs positions/matchs par fichier (xg, xgp, sgf, mat, txt, bgf).</td>
</tr>
<tr>
<td>delete, del, d</td>
...
<ul>
    ...
    <li>Les fichiers de match sont lus par <em>xgparser</em> et <em>gnubgparser</em> (LGPL-2.1) et par <em>bgfparser</em> (MIT).</li>
