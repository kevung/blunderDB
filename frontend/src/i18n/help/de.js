...
<p>Die Registerkarte <em>Bibliothek</em> vereint das, was zur geöffneten Datei gehört und nicht zum Rechner. Sie ist leer, solange keine Datenbank geöffnet ist, und sie sagt es.</p>
<p>Sie trägt zunächst die beiden <strong>Schwellenwerte</strong>, die über das Vokabular der gesamten Anwendung entscheiden: Eine Entscheidung ist ein <strong>Fehler</strong>, sobald ihre Kosten die Fehlerschwelle erreichen, und dieser Fehler ist ein <strong>Blunder</strong>, sobald sie die Blunder-Schwelle erreichen. Jeder Blunder ist ein Fehler, deshalb kann die erste Schwelle die zweite nicht überschreiten, und blunderDB lehnt das umgekehrte Paar ab. Die Werte werden in Equity eingegeben — „0,080“ — der Einheit aller Tabellen; die Befehlszeile hingegen spricht in Millipunkten, sodass die Schwelle 0,080 in einer Suche <code>E&gt;80</code> geschrieben wird.</p>
<p>Diese Schwellenwerte folgen der Datei, nicht dem Computer: Dieselbe Datenbank zählt überall, wo sie geöffnet wird, dieselben Blunders, <code>blunderdb info</code> zeigt sie an, und <code>blunderdb edit --error-threshold</code> / <code>--blunder-threshold</code> stellt sie ein. Sie reisen nicht in einem Export mit: Eine Schwelle ist eine Lesegewohnheit, keine Tatsache der Positionen.</p>
<p>Drei <strong>Voreinstellungen</strong> werden mit einem Klick angeboten, jede unter dem Namen des Programms, das diese Linie gezogen hat: blunderDB (0,050 / 0,100), XG (0,020 / 0,080) und gnubg (0,040 / 0,080). Standardmäßig liest eine Bibliothek 0,050 und 0,100.</p>
<p>Was sie auf dem Bildschirm ändern: die Anzahl der Blunders im Zähler der Statusleiste und die Suche, die dessen Link vorbereitet, die Spalten „Fehler“ und „Blunders“ der Statistiken und der Spielertabelle, die Markierungen der Züge in der Detailansicht eines Matches (Matches-Panel) sowie die Liste der Positionen, die blunderDB nach einem Import zur Durchsicht vorschlägt.</p>
...
<p>Zwei Einstellungen steuern die Ordnung der <strong>benachbarten Stellungen</strong> (Token <code>like</code>, siehe Such-Panel): die Anzahl der gelieferten Nachbarn und die maximale Entfernung, jenseits derer eine Stellung keine mehr ist. Diese Entfernung ist standardmäßig null, also keine Obergrenze: Der Maßstab hängt von der Spielphase ab, und ein hier gewählter Wert läse sich als Messung. Das Token <code>like&lt;12</code> setzt für eine Suche seine eigene, ohne die Einstellung anzurühren.</p>
...
<p>Die Registerkarte <em>Bibliothek</em> trägt schließlich <strong>Analysen reparieren</strong>: Die Analysespalten, welche die Suche und die Statistiken abfragen, sind eine Projektion der gespeicherten Analysen, die unangetastet bleiben. Ein Projektionsfehler lässt sich daher beheben, ohne irgendetwas neu zu importieren. Das geschieht ausdrücklich und niemals automatisch — die Analysespalten von jemandem allein deshalb neu zu schreiben, weil er seine Datenbank öffnet, ist nichts, was ein Werkzeug hinter seinem Rücken tun sollte. Dasselbe <code>blunderdb repair</code> steht in der Befehlszeile zur Verfügung.</p>
...
<p>Eine Stellung, eine Sammlung oder einen Kommentar zu löschen geht über einen <strong>Papierkorb</strong>: der Löschvorgang findet statt, aber eine Kopie dessen, was verschwindet, wird dreißig Tage aufbewahrt. Der Befehl <code>trash</code> öffnet das Fenster, das sie auflistet, jede mit <em>Wiederherstellen</em> und <em>Löschen</em>.</p>
...
<p>Im <strong>Bearbeitungsmodus</strong> nimmt <code>s like</code> das <strong>gezeichnete</strong> Brett als Ziel: Man zeichnet ungefähr die Stellung, an die man sich erinnert, startet, und die Bibliothek antwortet — dort, wo die Suche nach Struktur die genaue Zeichnung verlangt. Das Brett wird dann als Stellung gelesen und nicht als Muster: Ein leer gelassener Punkt zählt als ausgewürfelte Steine, was für eine echte Stellung genau ist und die Rechnung für eine halb fertige Zeichnung verfälscht.</p>
<p>Jede Nachbarstellung trägt ihre Entfernung unter den Analysetabellen, zusammen mit der Stellung, der sie nahe ist. Das erlaubt zu beurteilen, ob man eine Nachbarstellung oder einen Zufall betrachtet, und darin liegt der Sinn der Obergrenze. Die Ordnung lässt sich auch ohne die Kommandozeile starten: <em>STRG-UMSCHALT-L</em> oder der Eintrag <strong>Benachbarte Stellungen</strong> im Kontextmenü des Bretts.</p>
...
<p>Ein Klick auf ein Match öffnet seine Detailansicht. Deren Reiter <strong>Mitschrift</strong> listet die Züge Partie für Partie auf, und ein Klick auf einen Zug führt die Durchsicht dorthin. Jeder Zug trägt dort seine <strong>Schwere</strong>: <code>?</code> für einen Fehler, <code>??</code> für einen Blunder, eine farbige Linie am Rand der Zeile und die Kosten des Zuges in Equity, wenn man über die Markierung fährt. Die Schwellen sind die der Datenbank (Konfiguration), dieselben, mit denen die Statistiken zählen. Ein Zug wird so beurteilt, wie er gespielt wurde: Dieselbe Position, die im Match zweimal gespielt wurde, erhält zwei Urteile. Ein Zug, den die Analyse nicht bewertet, trägt keine Markierung.</p>
...
<h3>Transkriptions-Panel</h3>
<p>Das Panel <strong>Transkription</strong> (<em>CTRL-MAJ-T</em>, Befehl <code>transcribe</code> oder <code>tr</code>) dient dazu, ein Match einzutippen, das man vor sich hat — einen Matchbogen, eine Videoaufzeichnung —, um daraus ein Match der Bibliothek zu machen. Was eingetippt wird, ist ein <strong>Entwurf</strong>: Er lebt in der Datenbank, lässt sich schließen und wieder öffnen, und er geht weder in die Statistiken noch in die Suchen ein, solange er nicht als Match gespeichert wurde.</p>
<p>Das Panel öffnet sich mit der <strong>Liste der Entwürfe</strong> der Datenbank: letzte Änderung, Spieler, Matchlänge, Anzahl der Aktionen und das bereits erzeugte Match (<code>#</code> gefolgt von seiner Kennung) oder der Hinweis „kein Match“. Ein Klick öffnet einen Entwurf, und die Schaltfläche <strong>Entwürfe</strong> in der Leiste führt zurück. Die Schaltfläche <strong>Neue Transkription</strong> klappt das Erstellungsformular auf.</p>
<p>Die Liste lässt sich auch über die Tastatur durchlaufen: <em>AB</em> und <em>AUF</em> (oder <em>j</em> und <em>k</em>) bewegen die Hervorhebung, <em>EINGABE</em> öffnet den hervorgehobenen Entwurf, <em>n</em> klappt das Formular auf. Der erste Entwurf ist beim Öffnen hervorgehoben und es ist der zuletzt geänderte: die Arbeit von gestern wieder aufzunehmen kostet also zwei Tasten, <em>STRG-UMSCHALT-T</em> und dann <em>EINGABE</em>.</p>
<p>Das Formular fragt nur eines ab: die <strong>Matchlänge</strong>. Der Wert <code>0</code> steht für eine Geldpartie und blendet die Felder <em>Jacoby</em> und <em>Beaver</em> ein. Das Feld öffnet sich mit der Länge des zuletzt geänderten Entwurfs, oder mit 7, wenn die Datenbank keinen enthält. Spielernamen werden nicht abgefragt: Der Entwurf nennt die Seiten <em>Spieler 1</em> und <em>Spieler 2</em>, und die Liste zeigt „Ohne Namen“.</p>
<p>Alles, was sich aus dem Getippten ableitet — die Matchlänge (oder „Geld“), der Punktestand, der Hinweis <em>Crawford</em>, wenn die laufende Partie eine ist, die Partienummer, der Zustand des Dopplerwürfels — sein Wert, zentriert oder im Namen dessen, der ihn besitzt — und die Seite am Zug — erscheint in der <strong>Matchleiste</strong> über dem Brett: dort ist der Blick ohnehin, wenn man sich fragt, wer am Zug ist. Die erwartete Aktion steht ausgeschrieben in der <strong>Statusleiste</strong>: „Würfel von Kévin“, „Antwort von Alice auf das Doppel“, „neu würfeln“.</p>
<p>Die <strong>Entwurfsleiste</strong> am oberen Rand des Panels trägt nur die Gesten, die den Entwurf aus sich herausführen, die beiden Rückgängig-Pfeile und die Ausrichtung des Bretts.</p>
<p><strong>Spieler 1 bleibt unten am Brett</strong>, wer auch immer am Zug ist. Ein Match, das transkribiert wird, ist eine Partie im Verlauf: Der Zug wechselt bei jedem Halbzug, und ihm zu folgen würde das Brett von einem Zug zum nächsten umdrehen — die eben betrachteten Steine wanderten nach oben und das Auge müsste den Weg bei jedem Wurf neu gehen. Wer am Zug ist, liest man an den <strong>Würfeln</strong> ab, die die Seite wechseln. Die Schaltfläche ⇅ der Leiste dreht das Brett um und zeigt Spieler 2 unten; sie ändert den Entwurf nicht, und beim Schließen steht die Anzeige wieder richtig herum. Nicht zu verwechseln mit der Schaltfläche <em>Spieler tauschen</em> im Metadatenbereich, die die beiden Spieler im Dokument selbst vertauscht.</p>
<p>Die Schaltfläche <strong>Metadaten</strong> in der Leiste klappt jederzeit die Kopfdaten des Entwurfs auf: die Namen der beiden Spieler — automatisch aus den Spielern der Datenbank vervollständigt —, das Turnier, den Ort, die Runde, das Datum (standardmäßig das heutige), den Erfasser (standardmäßig der Benutzer der Datenbank) und die Turnierserie, der das Match beim Speichern zugeordnet wird. Kein Feld ist Pflicht: ein Entwurf ohne Namen wird ebenso gespeichert und exportiert, mit leeren Kopfzeilen. Die Schaltfläche <strong>Spieler vertauschen</strong> tauscht die beiden Namen, gibt alle Aktionen der Gegenseite und dreht das Brett um: es ist dasselbe Match, von der anderen Seite gelesen.</p>
<p>Die <strong>Matchlänge</strong> wird in derselben Ansicht geändert, jederzeit: der Punktstand, die Crawford-Partie und der Bezugsrahmen — Geldpartien, wenn die Länge <code>0</code> ist, wobei dann die Felder <em>Jacoby</em> und <em>Beaver</em> erscheinen — werden von einem Ende des Entwurfs bis zum anderen neu berechnet, und die nach dem Matchgewinn erfassten Aktionen werden als „nach dem Ende“ markiert, ohne dass eine einzige gelöscht wird. Die Länge gehört zur Identität einer Stellung: Wird sie nach einem Speichern geändert und erneut gespeichert, entstehen ganz neue Stellungen, die zu analysieren sind, und die alten verschwinden, sobald sie von nichts mehr gehalten werden.</p>
<p>Unter der Leiste belegt der Entwurf drei Bereiche: die <strong>Palette</strong> der Mausziele, die <strong>Zugkandidaten</strong> und das <strong>Transkript</strong>. Sie ordnen sich nach der Breite des Panels. In einem breiten Panel — dem unteren Dock — stehen alle drei nebeneinander, die Palette links. In einem mittleren Panel nehmen die Kandidaten den oberen Teil ein und das Transkript rückt neben das Wurfdreieck, in den Platz, den dieses rechts von sich frei lässt. In einem schmalen Panel folgen die drei aufeinander: Kandidaten, Palette, Transkript — Dreieck und Transkript passen dort nicht nebeneinander, ohne dem Transkript seine zweite Spalte zu nehmen, und das Dock um einige Dutzend Pixel zu verbreitern genügt, um sie zusammenzubringen. In allen Fällen gilt dieselbe Regel: Zwischen den beiden Wurffeldern und der ersten Kandidatenzeile steht nichts, und mindestens fünf Kandidaten lassen sich lesen, ohne irgendetwas zu scrollen.</p>
<p>Die Palette zeigt die beiden Würfel, während sie eingegeben werden; ein Klick darauf löscht sie, wie <em>RÜCKTASTE</em>. Eine Partie beginnt mit je einem Würfel pro Seite: die höhere beginnt und spielt beide Würfel, ohne sie erneut eingeben zu müssen; ein Gleichstand wird so festgehalten und eine weitere Eröffnung erwartet.</p>
...
<p>Der Spielstand einer Partie ist der, den die vorherigen Partien ergeben, es sei denn, er wurde am Tisch anders angesagt. Ein Doppelklick auf den Spielstand im Kopf einer Partie, im Transkript, verwandelt ihn in ein vorbelegtes Feld: Dort tippt man den Spielstand, zu dem die Partie gespielt wurde — <code>3-2</code>, <code>3–2</code> oder <code>3 2</code> —, EINGABE erfasst ihn, ESC schließt das Feld, ohne etwas zu schreiben, und ein geleertes, dann bestätigtes Feld kehrt zum abgeleiteten Spielstand zurück. Die Partie wird zu diesem Spielstand gespielt: Die Crawford-Partie, das Matchende und die folgenden Partien ergeben sich daraus, und das gespeicherte Match wie die <code>.mat</code>-Datei tragen ihn. Ein Spielstand, der vom abgeleiteten abweicht, wird markiert, der Tooltip nennt den abgeleiteten, und die Eröffnung der Partie trägt die Ungereimtheit „widersprüchlicher angesagter Spielstand“. Im Geldspiel gibt es keinen Spielstand anzusagen.</p>
<p>Das Transkript belegt die rechte Hälfte des Panels: eine Spalte je Spieler, eine Zeile je Zug, die Dopplerhandlung und das Partieende in der Spalte dessen, der handelt. Die Zelle des Cursors ist umrahmt; den Cursor zu bewegen führt das Brett zur Position der angezielten Aktion zurück und zeigt ihre Kandidaten, mit dem festgehaltenen Zug ausgewählt. Eine Ungereimtheit (illegaler Zug, doppelter Zug, unmögliche Dopplerhandlung, Aktion nach dem Matchende, widersprüchliche Würfel, nicht festgehaltener Zug, widersprüchlicher angesagter Spielstand) schmückt ihre Zelle und wird in einem Tooltip benannt. Der nicht festgehaltene Zug ist der Fall einer wieder eingelesenen <code>.mat</code>-Datei: gnubg schreibt dort <code>???</code>, wenn es den gespielten Zug nicht behalten hat, der Wurf ist bekannt und der Zug nicht, und den Cursor auf diese Zelle zu setzen bietet die Züge dieses Wurfs an, um ihn nachzutragen. Ein doppelter Zug hinterlässt eine leere, gestrichelt umrandete Zelle in der Spalte der Seite, deren Zug fehlt: Der Cursor hält dort an, ein Klick bringt ihn dorthin, und dort wird der fehlende Zug getippt — etwa eine gelöschte Entscheidung. Die Partien klappen zusammen; die des Cursors ist offen.</p>
<p><strong>Was gerade eingegeben wird, erscheint im Transkript</strong>, gestrichelt, genau an der Stelle, an der es geschrieben wird: die Würfel, sowie sie fallen, die Notation des Zuges, sobald er ausgewählt ist, und in der Spalte der Seite, der die Aktion gehört. Eine Korrektur überdeckt die Zelle, die sie ersetzt, eine Einfügung öffnet eine Zelle zwischen ihren beiden Nachbarn, eine neue Eingabe erscheint am Ende der laufenden Partie. Vor der Bestätigung wird nichts in den Entwurf geschrieben; was man liest und was das Dokument sagen wird, gehen nie auseinander.</p>
<p>Korrigieren heißt auf der Zelle tippen, auf der man steht. Steht der Cursor auf einer Aktion, beginnt eine Ziffer deren Wurf an Ort und Stelle neu, und auch die vier Dopplerhandlungen gelten als Korrekturen: Auf einer Aufgabe schreibt <em>t</em> — oder die Schaltfläche <strong>Annehmen</strong> — eine Annahme <strong>anstelle</strong> der Aufgabe, ohne sie löschen und dann einfügen zu müssen. Die Partie nimmt dann ihren Lauf wieder auf: direkt nach der Annahme öffnet sich eine Zelle auf der Seite des Verdoppelnden, und der Rest der Partie wird dort wie gewohnt eingegeben, eingefügt vor der Eröffnung der nächsten Partie, bis die Partie endet. Dieselben Tasten füllen die Zelle, die eine Einfügung gerade geöffnet hat: <em>i</em> und dann <em>d</em> fügt eine Verdopplung vor der angezielten Aktion ein.</p>
<p>Zur <strong>letzten</strong> Aktion zurückzukehren heißt, dorthin zurückzukehren, wo die Transkription geschrieben wird. Eine darauf getippte Ziffer korrigiert noch deren Wurf, doch sobald dieser Wurf neu getippt ist, bestätigt die nächste Ziffer sie und öffnet die darauffolgende Entscheidung; EINGABE bestätigt sie ebenso. Die folgenden Entscheidungen werden dann wie beim ersten Mal hinten angefügt.</p>
<p>Die beiden Würfel auf der <strong>Eröffnungszelle</strong> einer Partie erneut einzugeben entscheidet von Neuem, wer beginnt: der Würfel von Spieler 1 wird zuerst getippt, der von Spieler 2 danach, und der höhere gewinnt — der große Würfel zuerst, der Zug geht an Spieler 1 unten am Brett; der kleine zuerst, er geht an Spieler 2 oben. Die Bestätigung erfolgt sofort mit dem zweiten Würfel, wie bei einer neuen Eröffnung. <strong>Der erste Steinzug der Partie folgt dem neuen Gewinner</strong> und wechselt mit ihm die Spalte: seine Seite war nicht gewählt, sie war von der Eröffnung vorgeschlagen — der Gewinner des Wurfs spielt beide Würfel, ohne sie erneut einzugeben. Es ist die einzige Aktion, die eine korrigierte Eröffnung verschiebt: der Rest der Partie behält seine Seiten, und ein Zug, der mit <em>s</em> schon dem anderen Spieler gegeben wurde, wird nicht zurückgenommen.</p>
...
<p>Ein Vorschlag wird mit <strong>einem Klick</strong> auf <em>Starten</em> bestätigt. <strong>Alle starten</strong> bestätigt die ganze Liste in zwei Klicks, nachdem sie gezeigt wurde; <em>Bestätigen</em> steht am Kopf dieser Liste. „Vorerst übergehen“ schreibt nichts: Die Engine ist deterministisch, und der Vorschlag kommt beim nächsten Aufruf unverändert zurück. <em>Von Hand paaren</em> steht jederzeit offen — die Engine schlägt vor, die Leitung entscheidet.</p>
<p>Ein von Hand gepaartes Match ohne Tischnummer erhält den ersten freien Tisch. Ist der Saal voll, wird es trotzdem gestartet, und sein Feld erscheint am Ende des Rasters als „ohne Tisch“, bis es auf einen Tisch verschoben wird.</p>
...
<p>Vor <strong>Aufnehmen</strong> listet die Vorschau einer eingefügten CSV die unlesbaren Zeilen auf – ohne Namen, ohne Trennzeichen, wenn die anderen Zeilen eines haben, eine Wertung, die keine Zahl ist – sowie die Duplikate, innerhalb des Eingefügten oder mit einem bereits angemeldeten Spieler. Ein Duplikat wird nicht angemeldet, es sei denn, sein Kästchen wird angekreuzt.</p>
...
<p>Die Korrektur der Karte eines zurückgezogenen Spielers — Name, Verein, Wertung — lässt ihn zurückgezogen. Seine Rückkehr ist eine eigene Geste: <strong>Wieder aufnehmen</strong>, in seiner Zeile. Er wird wieder gepaart, mit den Ergebnissen und Leben, die er beim Rückzug hatte; die beim Rückzug durch Forfait verlorenen Matches bleiben verloren.</p>
...
<p>Der Reiter <strong>Wertung</strong> zeigt die laufende Wertung, Abschnitt für Abschnitt, mit den Preisen, sofern ein Preisgeld eingestellt ist. Zwei Gleichplatzierte teilen sich Platz und Preis. <strong>Turnier abschließen</strong> fixiert die Endwertung. Die Wertung wird als CSV kopiert, in der Sprache der Oberfläche, oder in einer Datei gespeichert: <strong>Speichern…</strong> öffnet den Dialog des Systems mit einem vorgeschlagenen Namen, dem Turnier gefolgt vom Wort „rangliste“ und dem heutigen Datum, und die Datei enthält genau die kopierte CSV. Auf der Kommandozeile schreibt <code>blunderdb tournament standings</code> dieselbe CSV.</p>
<p>Ohne laufendes Match ist das Abschließen ein Klick. Laufen noch Matches, nennt die Rangliste ihre Zahl und wartet vor Ort auf einen zweiten Klick: das Abschließen friert die Rangliste ohne sie ein, und ihr Ergebnis lässt sich nicht mehr eintragen. <strong>Wieder öffnen</strong> wird ebenso bestätigt; die Schlussrangliste ist dann nicht mehr endgültig, und das Wiederöffnen bleibt im Journal.</p>
...
<p><strong>Korrigieren</strong> in der Zeile eines Ergebnisses öffnet darunter dieselbe Rücknahme wie bei der letzten Entscheidung: den richtigen Sieger anklicken, bei Bedarf mit dem Ergebnis. Das ursprüngliche Ergebnis bleibt an seiner Stelle im Journal, die Korrektur kommt hinzu, und die Rangliste berücksichtigt sie sofort.</p>
...
<p>Hier wird eingestellt: die Phasen und ihre Matchlänge, die Längen Runde für Runde eines Tableaus („15, 13, 11“ liest sich von der letzten Runde rückwärts), die Anzahl der Tische, das geplante Tempo in Minuten pro Punkt (standardmäßig 8; Uhrleiste und voraussichtliches Ende gehen davon aus), die Pausen des Tages, das Preisgeld (Startgeld, Einbehalt des Vereins, Verteilung je Abschnitt) und der Anzeigeordner.</p>
<p>Ein <strong>Tableau</strong> hat drei Kästchen: <strong>Trostrunde</strong> (seine Verlierer spielen ein zweites Tableau, ein eigener Abschnitt in der Wertung), <strong>Versöhnung</strong> (der Sieger der Trostrunde spielt gegen den Sieger des Haupttableaus; das Kästchen erscheint nur mit der Trostrunde) und <strong>Nachladen</strong> (im Doppel-K.-o. muss der Sieger des Haupttableaus zweimal geschlagen werden; das Kästchen erscheint nur mit der Versöhnung). Eine Trostrunde hat nur mit einer Preisstaffel eine eigene Wertung: Solange die Staffel des Abschnitts <em>Trostrunde</em> leer ist, weisen die Einstellungen darauf hin. Eine Phase <strong>Gruppen</strong> wird über die Gruppengröße (standardmäßig 4) und die Zahl der Qualifizierten pro Gruppe (standardmäßig 2) eingestellt.</p>
<p>Die Einstellungen bleiben <strong>während des Turniers</strong> erreichbar: um 22 Uhr den Umschaltpunkt senken, um früher fertig zu werden, oder am Samstagabend eine Trostrunde hinzufügen, solange die Auslosung des Tableaus nicht gemacht ist. Was dann fixiert ist, ist mit seiner Begründung ausgegraut: das Format einer eröffneten Phase, die Anzahl der Leben, die sie vergeben hat, und ab der Auslosung einer Phase ihre Trostrunde, ihre Versöhnung, ihr Nachladen, ihre Gruppengröße und ihre Zahl der Qualifizierten. Ein Speichern während des Turniers zeigt zuerst die Liste dessen, was sich ändert, und fragt nach.</p>
<p>Ein Tisch mit kaputtem Brett wird unter <strong>Tische außer Betrieb</strong> eingetragen: seine Nummern, durch Kommas getrennt („7, 12“). Die Engine vergibt ihn nicht mehr, und das Raster zeigt ihn als nicht verfügbar; die Anzahl der Tische zu senken würde den letzten entfernen, nicht den kaputten. Läuft auf einem außer Betrieb gesetzten Tisch ein Match, sagt es die Liste der Änderungen und nennt einen freien Tisch, auf den es sich über die Matchkarte verlegen lässt.</p>
...
<tr>
<td>Blunders</td>
<td>Anzahl der Fehler, welche die Blunder-Schwelle der Bibliothek erreichen (standardmäßig 0,100 EMG).</td>
</tr>
<tr>
<td>Glück</td>
...
<p>Das Feld des Punktestands, den die Position tatsächlich trägt, ist umrahmt, und seine Zeilen- und Spaltenköpfe sind unterstrichen: Das Lesen beginnt dort, „mein Feld und alles darum herum“. Umrahmt wird es, sobald beide <em>Away</em>-Punktestände der Position in das angezeigte Raster passen; ein Wechsel der Länge verschiebt den Rahmen oder entfernt ihn. Eine Money-Position, das Crawford-Spiel oder ein <em>Away</em> jenseits des Rasters bezeichnen kein Feld: Es gibt kein Feld zu zeigen, und ein ungefähres zu zeigen wäre falsch.</p>
...
<p>In einer OGID erkennt man das Crawford-Spiel am <code>C</code> hinter der Matchlänge (<code>7C</code>): Fehlt es, ist ein Spieler, dem ein Punkt zum Sieg fehlt, bereits nach dem Crawford-Spiel.</p>
...
<p>Das <strong>Anki</strong>-Panel wiederholt, was <strong>behalten</strong> wird; das <strong>Training</strong>-Panel übt, was <strong>gerechnet</strong> wird, unter Zeitdruck. Es öffnet sich mit <code>CTRL-J</code>, über die Schaltfläche der Werkzeugleiste direkt nach « Position aléatoire » oder über den Befehl <code>train</code>. Die Standkarte gehört zu beiden: Sie wird hier gerechnet und in einem Stapel mit Standkarten behalten.</p>
<p>In Ruhe zeigt das Panel den Starter und die Bilanz der vergangenen Sitzungen.</p>
<h4>Der Starter</h4>
<p>Drei Auswahlen, dann « Démarrer »:</p>
<ul>
<li>die <strong>Übung</strong> — <em>Scores</em>, <em>Pions</em> (Punktzahl), <em>Bearoff</em>, <em>Évaluation</em> oder <em>Décision</em>;</li>
<li>die <strong>Quelle</strong> der Frage, wenn die Übung mehrere hat — <em>Vivier</em> (kanonische Formen der Übung), <em>Plateau</em> (die Stellung, wie sie ist) oder <em>Base</em> (eine Stellung der durchsuchten Liste);</li>
...
<p>Jede Frage wird <strong>erzeugt</strong>: die Engine geht von einem Saatkorn aus und spielt einige Würfe, und der Schnappschuss ist die Frage. Eine zufällige Platzierung hätte nicht die Lücken, die niedrigen Stapel und die Asymmetrien eines echten Auswürfelns. Das Saatkorn kommt aus dem <em>Vivier</em> (eine abgeschlossene Einbringung, dann null bis zehn Halbzüge), vom <em>Plateau</em> (die angezeigte Stellung, dann ein bis vier Halbzüge — nie null, denn Sie haben sie gerade gesehen) oder aus der <em>Base</em> (eine Stellung der durchsuchten Liste, so wie sie ist: sie ist bereits echt).</p>
<p>Der Bereich der Übung: <strong>beide Seiten vollständig im Heimfeld</strong>, <strong>4 bis 15 Steine</strong> je Seite und der Rest ausgewürfelt, Dopplerwürfel in der Mitte, Geldspiel. Ein Saatkorn, das nicht hineinpasst, wird <strong>unter Nennung des Grundes abgelehnt</strong>, und nichts startet — keine stille Anpassung: weiterzuspielen, bis der Kontakt bricht, gäbe Ihnen eine Stellung, die Sie nicht gewählt haben. Ein leeres Brett ist die Ausnahme: die Frage kommt dann aus dem Vorrat, und das Panel sagt warum.</p>
<p>Die Übung braucht die einseitige Bearoff-Tabelle; solange sie im Hintergrund erzeugt wird (siehe Konfiguration), sagt sie das, statt eine Frage ohne Antwort zu stellen.</p>
<p><strong>Évaluation</strong> fragt, was eine Stellung wert ist: die <strong>Gewinnchance des Spielers am Zug</strong> in Prozent und die <strong>Doppelentscheidung</strong> — <em>Pas de double</em>, <em>Double, prend</em> oder <em>Double, passe</em>. Das sind die beiden Zahlen, die das Eval-Panel zeigt, abgefragt, bevor sie gezeigt werden. Der Bereich ist <strong>jede Stellung</strong>: ein Rennen ebenso wie eine Kontaktstellung, im <strong>Geldspiel</strong>.</p>
<p>Wie bei Bearoff wird die Frage <strong>erzeugt</strong>: Der Keim kommt aus dem <em>Vivier</em> (eine Stellung, in der der Kontakt gerade aufgelöst ist, dann null bis zehn von der Engine gespielte Halbzüge), vom <em>Plateau</em> (die angezeigte Stellung, dann ein bis vier Halbzüge; die Frage wird im Geldspiel mit dem Doppler in der Mitte gestellt, gleich welchen Spielstand der Keim trägt) oder aus der <em>Base</em> (eine Stellung der durchsuchten Liste, so wie sie ist). Eine Stellung aus der Datenbank eignet sich nur, wenn sie eine Doppelentscheidung im Geldspiel ist — ohne Würfel, der Doppler in der Mitte oder beim Spieler am Zug —; sonst geht die Ziehung zur nächsten über, und wenn keine passt, sagt die Übung es. Ein Brett, das keine Spielstellung ist — nicht fünfzehn Steine je Seite, oder eine beendete Partie —, wird <strong>mit Begründung abgelehnt</strong>; bei einem leeren Brett kommt die Frage aus dem Vorrat.</p>
<p>Die Wahrheit ist die der Engine: die zweiseitige Bearoff-Datenbank, wenn die Stellung darin steht, gammonNet in seiner kanonischen Tiefe überall sonst, und das Panel sagt, welche geantwortet hat. Nichts wird geschätzt: Eine Stellung, die die Engine nicht bewertet, wird nicht gestellt.</p>
<p><strong>Décision</strong> stellt eine <strong>bereits analysierte</strong> Entscheidung: eine Stellung der durchsuchten Liste — ihre einzige Quelle — mit der Entscheidung, die sie trägt, Zug oder Doppelentscheidung, und die gespeicherte Analyse urteilt. Eine Stellung ohne Analyse stellt keine Frage, und eine gestellte Stellung kehrt in der Sitzung nicht wieder; ist die Liste erschöpft, sagt es das Panel. Ohne geöffnete Datenbank oder ohne analysierte Stellung in der Liste <strong>verweigert die Übung und sagt warum</strong>, und nichts startet.</p>
<p>Solange eine Frage von <em>Évaluation</em> oder <em>Décision</em> auf ihre Antwort wartet, bleibt das Analyse-Panel verdeckt: Es trägt die Antwort.</p>
<h4>Antworten</h4>
<p>Der Antwortmodus ist eine Eigenschaft der Übung, nie eine Einstellung: was <strong>gezählt oder auswendig gewusst</strong> wird, wird aufgedeckt, was <strong>geschätzt</strong> wird, wird eingegeben — denn dort ist die Größe des Fehlers die Lektion.</p>
<p><em>Scores</em> und <em>Pions</em> werden <strong>aufgedeckt</strong>: Sie rechnen im Kopf, Sie klicken « Révéler », und die Wahrheit erscheint. Jede Zahl gilt dann <strong>standardmäßig als richtig</strong> — Sie klicken die an, die Sie verfehlt haben, um sie als <strong>Fehler</strong> zu markieren (<em>Tab</em> und dann <em>Leertaste</em> macht dasselbe über die Tastatur), und ein zweiter Klick hebt die Markierung auf. Nichts wird getippt: eine Punktzahl oder ein Tabellenfeld ist richtig oder falsch, und es aufzuschreiben lehrt nichts, was das Lesen nicht schon lehrt.</p>
<p><em>Bearoff</em> wird <strong>eingegeben</strong>: Sie schreiben die beiden EPC, « Valider » bewertet sie auf einen halben Punkt genau — die Granularität, bei der der EPC eine Rennentscheidung ändert — und die Wahrheit erscheint neben dem, was Sie geschrieben haben. Die Anwendung urteilt, es gibt nichts anzuklicken. Die Abweichung wird <strong>mit ihrem Vorzeichen</strong> aufgezeichnet: überschätzen ist nicht unterschätzen, und die Bilanz macht daraus einen Mittelwert.</p>
<p><em>Évaluation</em> verbindet beide Gesten in einer einzigen Frage. Die Gewinnchance wird <strong>eingegeben</strong> und auf fünf Punkte genau bewertet, samt vorzeichenbehafteter Abweichung; die Doppelentscheidung wird <strong>gewählt</strong> — der Klick merkt sich die Schaltfläche, ohne etwas zu bewerten, und « Valider » bewertet beides auf einmal. Der Doppler hat keine Toleranz: Nur die Schaltfläche, die das Urteil der Engine richtig macht, ist richtig, und eine Stellung, die <em>zu gut zum Doppeln</em> ist, wird mit <em>Pas de double</em> beantwortet. <em>Eingabe</em> im Feld führt zur Doppelentscheidung, solange sie nicht gewählt ist, und bestätigt dann. Nach der Antwort zeigt das Panel die Wahrheit — das Urteil mit vier Ausgängen —, ihre Quelle und den <strong>EPC beider Seiten</strong>, wenn die Stellung einen exakten hat; der EPC wird hier nie abgefragt, er hat seine eigene Übung. Das Journal zählt die beiden Zahlen getrennt: Man kann eine Stellung gut schätzen und ihren Doppler falsch lesen.</p>
<p><em>Décision</em> wird <strong>gewählt</strong>. Bei einer Zugentscheidung <strong>spielen Sie den Zug auf dem Brett</strong>: Klicken Sie den Ausgangspunkt, dann das Ziel, oder ziehen Sie den Stein, einmal je Würfel. Das Brett bietet nur an, was spielbar ist — ein Klick, den kein legaler Zug erlaubt, bewegt nichts. Im Panel nimmt « Annuler le pas » einen Würfel zurück, « Recommencer » stellt die Stellung so wieder her, wie die Frage sie stellt (ein Doppelklick außerhalb des Bretts tut dasselbe), und « Valider », aktiv sobald der Zug vollständig ist, lässt ihn bewerten. Bei einer Verdopplungsentscheidung klicken Sie <em>Kein Doppel</em>, <em>Doppel, Annahme</em> oder <em>Doppel, Aufgabe</em>: Der Klick ist die Antwort.</p>
<p>Die Korrektur hält drei Ausgänge auseinander, und sie zu vermengen wäre gelogen. Ein <strong>unerlaubter Zug</strong> ist kein schlecht gewählter Zug — er ist ein Regelfehler. Ein <strong>erlaubter Zug, den die Engine nie bewertet hat</strong>, ist kein Urteilsfehler: Er hat schlicht keinen Preis und kostet nichts. Ein bewerteter Zug kostet, was die Analyse sagt, in Millipunkten. Richtig ist nur ein bewerteter Zug ohne Kosten; der beste Zug wird in jedem Fall angezeigt.</p>
<p>Die Uhr läuft ab dem Erscheinen der Frage und hält bei « Révéler », bei « Valider » oder beim Klick auf eine Doppelentscheidung von <em>Décision</em>; die nächste Frage wird vorbereitet, während Sie antworten, sie wird also nie mit Ihrer zusammen gemessen. Das Ankreuzen der Fehler wird ebenfalls nicht gemessen. Mit einem Limit deckt sich eine bei Ablauf unbeantwortete Frage selbst auf und zählt als <strong>überfällig</strong>: alle ihre Zahlen sind falsch, und ihre Zeit geht nicht in den Median ein — man misst keine Antwort, die nicht gegeben wurde. Eine überfällige Entscheidung zeigt den besten Zug und geht nicht in den PR der Sitzung ein.</p>
...
<p>Solange eine Frage auf dem Brett steht, aufgedeckt oder nicht, blättern die Tasten, die die Liste durchlaufen, sie nicht weiter: Die Frage behält das Brett bis « Suivante », « Terminer » oder « Quitter ».</p>
...
<p>Solange eine Frage des Trainings-Panels auf dem Brett steht, blättern die Tasten, die die Liste durchlaufen, sie nicht weiter: Die Frage behält das Brett.</p>
<h3>Anzeige</h3>
<table>
<thead>
<tr>
...
<td>Die Nachbarstellungen der aktuellen Stellung ordnen.</td>
</tr>
<tr>
<td>STRG-P</td>
...
<p>Das Panel nimmt diese Tasten, solange es den Fokus hat. Vor der Liste der Entwürfe nimmt es bereits einige, damit die Arbeit von gestern nicht die Maus verlangt.</p>
<table>
<thead>
<tr>
<th>Tastenkürzel</th>
...
<tr>
<td>EINGABE (Liste der Entwürfe)</td>
<td>Den hervorgehobenen Entwurf öffnen.</td>
</tr>
<tr>
<td>n (Liste der Entwürfe)</td>
<td>Das Erstellungsformular öffnen.</td>
</tr>
<tr>
<td>Klick</td>
...
<td>Den ausgewählten Zug bestätigen und den nächsten Wurf öffnen. Bei einer erneut betrachteten Aktion, wo der Cursor auf einer bereits geschriebenen Aktion steht, beginnt die Ziffer den Wurf dieser Aktion neu, statt zu bestätigen.</td>
</tr>
<tr>
<td>1 … 6 (Cursor auf der Eröffnung einer Partie)</td>
<td>Den Eröffnungswurf erneut eingeben: Würfel von Spieler 1, dann von Spieler 2, mit dem zweiten Würfel festgehalten. Der höhere gewinnt — der große Würfel zuerst, der Zug geht an Spieler 1 unten; der kleine zuerst, an Spieler 2 oben. Der erste Steinzug der Partie folgt dem neuen Gewinner.</td>
</tr>
<tr>
<td>UNTEN, j</td>
...
<tr>
<td>Mausrad</td>
<td>Den nächsten oder vorherigen Kandidaten wählen, über der Liste ebenso wie über dem Brett: der Blick bleibt auf dem Brett und die Pfeile ziehen vorbei.</td>
</tr>
<tr>
...
<tr>
<td>Doppelklick (auf eine Zeile)</td>
<td>Diesen Kandidaten bestätigen.</td>
</tr>
<tr>
...
<tr>
<td>RÜCKTASTE</td>
<td>Die beiden eingegebenen Würfel löschen. Darüber führt der Weg zurück zu einem falsch gelesenen Wurf, da eine Ziffer bestätigt.</td>
</tr>
<tr>
<td>Klick (auf die Würfelfelder)</td>
<td>Die beiden eingegebenen Würfel löschen, wie RÜCKTASTE.</td>
</tr>
<tr>
<td>Esc</td>
...
<tr>
<td>t</td>
<td>Die angebotene Verdopplung annehmen: der Dopplerwürfel geht zum verdoppelten Wert an den Annehmenden, und der Verdoppelnde würfelt erneut. Steht der Cursor auf einer Zelle, wird die Annahme anstelle der angezielten Aktion geschrieben; eine zur Annahme gewordene Aufgabe öffnet ihre Partie wieder, und der Rest wird stattdessen dort eingegeben.</td>
</tr>
<tr>
<td>p</td>
<td>Die angebotene Verdopplung aufgeben: die Partie wird zum Wert vor der Verdopplung gewonnen. Steht der Cursor auf einer Zelle, wird die Aufgabe anstelle der angezielten Aktion geschrieben.</td>
</tr>
<tr>
...
<tr>
<td>Klick (auf den Doppler)</td>
<td>Für die Seite am Zug ein Doppel anbieten, wie die Taste d. Vor einem Angebot antwortet der Doppler nicht: Annehmen und Passen stehen in der Schaltflächenreihe.</td>
</tr>
<tr>
<td>LINKS, h</td>
...
<td>Den Cursor auf diese Aktion setzen, oder auf den fehlenden Zug eines doppelten Zugs.</td>
</tr>
<tr>
...
<tr>
<td>Doppelklick (auf den Spielstand einer Partie)</td>
<td>Den Spielstand, zu dem diese Partie gespielt wurde, in ihrem Kopf tippen: 3-2, 3–2 oder 3 2. EINGABE erfasst ihn, ein geleertes Feld kehrt zu dem Spielstand zurück, den die vorherigen Partien ergeben, und Esc schließt das Feld, ohne etwas zu schreiben. Ein Spielstand, der von diesem abweicht, wird als Ungereimtheit markiert. Kein Spielstand im Geldspiel.</td>
</tr>
<tr>
<td>Rechtsklick (auf eine Zelle)</td>
<td>Die Korrekturen dieser Aktion öffnen: davor einfügen, danach einfügen, löschen, Seite wechseln. Der Cursor wird dabei auf die Zelle gesetzt.</td>
</tr>
<tr>
<td>STRG-EINGABE</td>
...
<tr>
<td>x, Entf</td>
<td>Die gerade bearbeitete Entscheidung löschen — die Aktion des Cursors oder die noch nicht geschriebene Eingabe — und auf die vorige zurückgehen, bereit zur Korrektur; die folgenden behalten ihre Seite. Am Ende des Dokuments geht es auf die letzte Aktion zurück.</td>
</tr>
<tr>
<td>s</td>
...
<p>Eine Geste, die nichts zu tun hat, sagt es einmal in der Statusleiste: „nichts rückgängig zu machen“ bei leerem Stapel, „keine Aktion unter dem Cursor“ am Ende des Dokuments. Der Satz verschwindet von selbst und gibt den Platz an die erwartete Aktion zurück.</p>
...
<p>Nichts wird abgelehnt und nichts gelöscht: Eine Aktion derselben Seite wie ihre Nachbarin einzufügen erzeugt einen doppelten Zug, eine Aktion zu löschen kann einen weiteren erzeugen, eine Seite zu ändern kann die folgenden Züge ungültig machen. Diese Unstimmigkeiten werden im Transkript markiert, niemals von selbst behoben, und der Cursor springt nach jeder schreibenden Geste auf die erste von ihnen — außer dort, wo man weitermacht: nach einem Löschen, einer Einfügung oder einer wieder geöffneten Partie bleibt er, wo man tippt. Sich zu bewegen oder einen Würfel zu tippen verschiebt ihn nie. Der Zug, den ein doppelter Zug verloren hat, ist eine Zelle des Transkripts: h und l halten dort an, und ein dort getippter Wurf wird für die Seite eingefügt, der er fehlte. Der Rückgängig-Stapel liegt im Arbeitsspeicher: Er geht verloren, sobald der Entwurf geschlossen wird.</p>
