...
<p>Das Panel öffnet sich auf der <strong>Liste der Entwürfe</strong> der Datenbank: Datum der letzten Änderung, Spieler, Länge, Anzahl der Aktionen und das bereits erzeugte Match (<code>#</code> gefolgt von seiner Kennung) oder der Vermerk „nicht gespeichert“. Ein Klick öffnet einen Entwurf, und die Schaltfläche <strong>Entwürfe</strong> der Leiste führt zur Liste zurück. Die Schaltfläche <strong>Neue Transkription</strong> klappt das Anlageformular auf.</p>
...
<p>Ein geöffneter Entwurf trägt oben seine <strong>Leiste</strong>, die alles nennt, was sich aus dem Eingetippten ergibt: die Matchlänge (oder „Geldspiel“), den Punktestand, den Vermerk <em>Crawford</em>, wenn die laufende Partie eine ist, die Partienummer, den Zustand des Dopplerwürfels — seinen Wert, zentriert oder auf den Namen dessen, der ihn besitzt — und die Seite am Zug.</p>
...
<p>Unter der Leiste nimmt der Entwurf zwei Hälften ein: links die <strong>Eingabe</strong>, rechts das <strong>Transkript</strong>. Darüber liegt eine Korrekturleiste — davor einfügen, danach einfügen, löschen, Seite wechseln, rückgängig, wiederherstellen —, die die Tasten des Panels mit der Maus aufnimmt.</p>
<p>Die linke Hälfte kündigt an, was sie erwartet, und zeigt die beiden Würfel, während sie eingegeben werden. Eine Partie beginnt mit je einem Würfel pro Seite: Der höhere beginnt und spielt beide Würfel, ohne sie erneut eingeben zu müssen; ein Gleichstand wird so aufgezeichnet, wie er ist, und eine weitere Eröffnung wird erwartet.</p>
<p>Sobald der zweite Würfel fällt, werden alle <strong>gültigen Züge</strong> des Wurfs aufgelistet, von der eingebauten Engine sortiert, der erste vorausgewählt und seine Pfeile auf dem Brett gezeigt. Diese Sortierung ist eine Bewertung: Sie wird angezeigt, sie wird nie in die Datenbank geschrieben. Ist die Engine nicht verfügbar, werden die Züge ohne Sortierung aufgelistet, und das Panel sagt es.</p>
...
<p>Ein Klick auf einen Punkt des Bretts lässt nur die Züge übrig, die von ihm ausgehen. Das ist die Geste für einen weit unten stehenden Zug: bis zum zwölften Kandidaten sind es dreizehn Tastendrücke, mit dem Filter nur noch zwei oder drei. Der Filter ändert nichts am Entwurf, er verkürzt die Liste auf dem Bildschirm; der nächste Wurf hebt ihn auf.</p>
...
<p>Ein illegaler Zug wird so erfasst, wie er gespielt wurde. Die Schaltfläche « Freies Versetzen » gibt das Brett frei: Die Steine bewegen sich ohne jede Prüfung, und « Dieses Brett ist der gespielte Zug » erfasst das erhaltene Brett. Das Notationsfeld daneben tut dasselbe über die Tastatur: <code>13/7 8/7*</code>, <code>bar/22</code> oder <code>6/off</code> werden geschrieben und mit EINGABE erfasst. Beide verlangen, dass zuerst die Würfel des Wurfs eingegeben werden, da ein illegaler Zug nicht sagt, welcher Wurf ihn hervorgebracht hat. Ein über einen dieser beiden Wege erfasster Zug, der sich als legal erweist, bleibt ein gewöhnlicher Zug — verglichen wird das erhaltene Brett, nie die Herkunft der Geste; sonst wird er im Transkript als « illegaler Zug » markiert, und der <code>.mat</code>-Export warnt, bevor er die Datei schreibt, ohne sich je zu verweigern.</p>
<p>Unter dem Dreieck bringt die Reihe <strong>Doppeln</strong>, <strong>Annehmen</strong>, <strong>Passen</strong>, <strong>Aufgeben</strong> die vier Dopplergesten auf die Maus. Sie sagt, wer an der Reihe ist: die Seite am Zug kündigt an — doppeln, aufgeben — oder die Gegenseite antwortet — annehmen, passen; nie alle vier zugleich, und eine Schaltfläche, deren Geste auf nichts antworten würde, bleibt ausgegraut. Die Tastatur dagegen verweigert nie etwas: eine ausgegraute Schaltfläche ist ein Ziel, das nicht angeboten wird, kein verbotener Zug. „Aufgeben“ zeichnet noch nichts auf: die Reihe wird zu den drei Stufen — einfach, Gammon, Backgammon — und „Abbrechen“, das die Taste ESC aufnimmt. Der auf dem Brett gezeichnete Doppler ist das zweite Ziel dieser Gesten: ein Klick darauf bietet ein Doppel an. Auf ein Angebot antwortet er nicht — Annehmen und Passen sind zwei symmetrische Antworten und wohnen gemeinsam in der Reihe, je ein Klick.</p>
<p>Unter den Würfeln nennt eine Zeile den Stand der Eingabe: den noch korrigierbaren Wurf, den gewählten Kandidaten, den von selbst erfassten Tanz, den erneut zu würfelnden Gleichstand, die erwartete Antwort auf ein Doppel, die nach einer Resignation erwartete Stufe, die Korrektur an Ort und Stelle, den Zug „zu prüfen“, dessen Wurf sich geändert hat, und die Unstimmigkeit, welche die letzte Aktion hinterlassen hat.</p>
...
<p>Das Transkript nimmt die rechte Hälfte des Panels ein: eine Spalte je Spieler, eine Zeile je Zug, die Dopplerabgabe und das Partieende in der Spalte dessen, der handelt. Die Zelle des Cursors ist umrahmt; wird der Cursor bewegt, zeigt das Brett wieder die Position der angesteuerten Aktion und deren Kandidaten, wobei der gespielte Zug ausgewählt ist. Eine Unstimmigkeit (unerlaubter Zug, doppelter Zug, unmögliche Dopplerabgabe, Aktion nach dem Matchende, unstimmige Würfel, nicht aufgezeichneter Zug) markiert ihre Zelle und wird in einem Tooltip benannt. Ein nicht aufgezeichneter Zug stammt aus einer wieder eingelesenen <code>.mat</code>-Datei: gnubg schreibt dort <code>???</code>, wenn es den gespielten Zug nicht behalten hat; der Wurf ist bekannt, der Zug nicht, und wenn der Cursor auf diese Zelle gesetzt wird, werden die Züge dieses Wurfs angeboten, um ihn nachzutragen. Partien lassen sich einklappen; die Partie des Cursors ist offen. Ein aufklappbarer Bereich zeigt den exakten <code>.mat</code>-Text des Entwurfs mit einer Schaltfläche zum Kopieren.</p>
<p>Ein Rechtsklick auf eine Zelle öffnet die Korrekturen dieser Aktion — davor einfügen, danach einfügen, löschen, Seite wechseln — und bringt den Cursor dabei auf sie; es sind dieselben Gesten wie in der Korrekturleiste und auf den Tasten, und nur dort wird das Browsermenü unterdrückt.</p>
