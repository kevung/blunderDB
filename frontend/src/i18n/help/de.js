...
<h3>Anki-Panel</h3>
<p>Das Panel <strong>Anki</strong> (<em>CTRL-K</em>) ermöglicht das Lernen durch verteiltes Wiederholen mithilfe des FSRS-Algorithmus. Eine Karte stellt eine Frage: meist eine Stellung, aus einer Sammlung oder einer Suche; es kann auch ein Spielstand sein.</p>
...
<p><strong>Wiederholung:</strong> Wählen Sie einen Stapel aus und klicken Sie dann auf <em>Study</em> (oder doppelklicken Sie auf einen Stapel), um mit der Wiederholung der fälligen Karten zu beginnen. Eine Stellungskarte zeigt die Stellung auf dem Brett; eine Standkarte nennt den Stand und lässt das Brett, wie es ist. Bewerten Sie Ihre Erinnerung mit den Tasten <em>1</em> (Nochmal), <em>2</em> (Schwierig), <em>3</em> (Gut) oder <em>4</em> (Einfach). Drücken Sie <em>Esc</em>, um anzuhalten und zur Stapelliste zurückzukehren.</p>
...
<p><strong>Antwort anzeigen:</strong> Die Karte stellt eine Frage — welcher Zug zu spielen ist, welche Doppler-Aktion, oder welche Zahlen ein Stand trägt. Überlegen Sie, und drücken Sie dann <em>LEERTASTE</em> (oder klicken Sie auf den verdeckten Bereich), um die Antwort aufzudecken: die gespeicherte Analyse der Stellung, so wie der Tab Analyse sie darstellt, oder das Standblatt, vollständig. Auf einem Blatt gibt es nichts anzukreuzen: Anki plant ein Gedächtnis, es misst keine Rechnung — die Fehler zählt das Training. Sie erscheint unter den Bewertungsschaltflächen, die an ihrem Platz und in Reichweite bleiben. Ein Klick auf einen Zug der Liste zeigt ihn auf dem Brett.</p>
...
<p>Das <strong>Anki</strong>-Panel wiederholt, was <strong>behalten</strong> wird; das <strong>Training</strong>-Panel übt, was <strong>gerechnet</strong> wird, unter Zeitdruck. Es öffnet sich mit <code>CTRL-J</code>, über die Schaltfläche der Werkzeugleiste direkt nach « Position aléatoire » oder über den Befehl <code>train</code>. Das Standblatt gehört zu beidem: hier wird es gerechnet, und in einem Stapel mit Standkarten wird es behalten.</p>
