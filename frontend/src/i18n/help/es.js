...
<p>El marcador de una partida es el que dan las partidas anteriores, salvo si se anunció otro en la mesa. Un doble clic en el marcador del encabezado de una partida, en la transcripción, lo convierte en un campo rellenado: allí se teclea el marcador con el que se jugó la partida — <code>3-2</code>, <code>3–2</code> o <code>3 2</code> —, INTRO lo registra, ESCAPE cierra el campo sin escribir nada, y un campo vaciado y luego validado vuelve al marcador derivado. La partida se juega con ese marcador: la partida Crawford, el final del partido y las partidas siguientes se derivan de él, y tanto el partido guardado como el archivo <code>.mat</code> lo llevan. Un marcador que difiere del derivado se marca, la información emergente da el derivado, y la apertura de la partida lleva la incoherencia «marcador anunciado incoherente». En dinero no hay marcador que anunciar.</p>
...
<p>Corregir es teclear en la celda donde se está. Con el cursor sobre una acción, una cifra vuelve a empezar su tirada en el sitio, y las cuatro acciones de cubo valen también como correcciones: sobre un rechazo, <em>t</em> — o el botón <strong>Aceptar</strong> — escribe una aceptación <strong>en lugar</strong> del rechazo, sin tener que borrarlo y luego insertar. La partida retoma entonces su curso: se abre una celda justo después de la aceptación, en el bando del que dobló, y el resto de la partida se teclea allí como de costumbre, insertado delante de la apertura de la partida siguiente, hasta que la partida termina. Las mismas teclas rellenan la celda que acaba de abrir una inserción: <em>i</em> y luego <em>d</em> inserta un doble delante de la acción señalada.</p>
...
<p>Volver a introducir los dos dados en la celda de <strong>apertura</strong> de una partida decide de nuevo quién empieza: el dado del jugador 1 se teclea primero, el del jugador 2 después, y gana el mayor — el dado grande primero, el turno va al jugador 1, abajo en el tablero; el pequeño primero, va al jugador 2, arriba. La validación es inmediata con el segundo dado, como para una apertura nueva. <strong>El primer movimiento de fichas de la partida sigue al nuevo ganador</strong> y cambia de columna con él: su bando no había sido elegido, lo proponía la apertura — el ganador de la tirada juega ambos dados sin volver a introducirlos. Es la única acción que desplaza la corrección de una apertura: el resto de la partida conserva sus bandos, y un movimiento ya dado al otro jugador con <em>s</em> no se recupera.</p>
...
<p>Una propuesta se confirma con <strong>un clic</strong> en <em>Lanzar</em>. <strong>Lanzar todo</strong> confirma la cola entera en dos clics, tras mostrar su lista; <em>Confirmar</em> está en la cabecera de esa lista. «Ignorar por ahora» no escribe nada: el motor es determinista, y la propuesta vuelve idéntica en la siguiente llamada. <em>Emparejar a mano</em> está siempre disponible — el motor propone, el director decide.</p>
<p>Un match emparejado a mano sin número de mesa toma la primera mesa libre. Si la sala está llena, se lanza igualmente y su casilla aparece al final de la cuadrícula, «sin mesa», hasta que se le traslade a una mesa.</p>
...
<p>La pestaña <strong>Clasificación</strong> muestra la clasificación actual, sección por sección, con los premios cuando hay una dotación configurada. Dos empatados comparten el puesto y el premio. <strong>Cerrar el torneo</strong> fija la clasificación final. La clasificación se copia en CSV, en el idioma de la interfaz, o se guarda en un archivo: <strong>Guardar…</strong> abre el diálogo del sistema con un nombre propuesto, el torneo seguido de la palabra «clasificacion» y la fecha del día, y el archivo contiene exactamente el CSV copiado. En la línea de comandos, <code>blunderdb tournament standings</code> escribe el mismo CSV.</p>
<p>Cerrar sin matches en curso es un clic. Con matches en curso, la Clasificación dice cuántos y espera un segundo clic en el sitio: cerrar congela la clasificación sin ellos, y su resultado ya no se puede anotar. <strong>Reabrir</strong> se confirma de la misma manera; la clasificación final deja entonces de ser final, y la reapertura queda en el registro.</p>
...
<p>Aquí se ajustan: las fases y la longitud de sus partidos, las longitudes ronda a ronda de un cuadro («15, 13, 11» se lee de la última ronda hacia atrás), el número de mesas, el ritmo previsto en minutos por punto (8 por defecto; la banda del reloj y el fin estimado parten de él), las pausas del día, la dotación (inscripción, retención del club, baremo por sección) y la carpeta de la pantalla.</p>
...
<p>Los ajustes siguen accesibles <strong>durante el torneo</strong>: bajar la báscula a las 22 h para terminar antes, añadir una consolación el sábado por la noche, mientras no se haya hecho el sorteo del cuadro. Lo que queda entonces congelado aparece atenuado con su motivo: el formato de una fase abierta, el número de vidas que ha repartido y, desde el sorteo de una fase, su consolación, su reconciliación, su recarga, el tamaño de sus grupos y su número de clasificados. Guardar durante el torneo muestra primero la lista de lo que va a cambiar y pide confirmación.</p>
...
<ul>
<li>el <strong>ejercicio</strong> — <em>Scores</em>, <em>Pions</em> (recuento), <em>Bearoff</em>, <em>Évaluation</em> o <em>Décision</em>;</li>
...
<p>Mientras una pregunta del panel Entrenamiento está planteada en el tablero, las teclas que recorren la lista no la hacen avanzar: la pregunta conserva el tablero.</p>
<h3>Visualización</h3>
<table>
<thead>
<tr>
...
<tr>
<td>1 … 6 (cursor en la apertura de una partida)</td>
<td>Volver a introducir la tirada de apertura: dado del jugador 1 y luego del jugador 2, registrados con el segundo dado. Gana el mayor — el dado grande primero, el turno va al jugador 1 abajo; el pequeño primero, al jugador 2 arriba. El primer movimiento de fichas de la partida sigue al nuevo ganador.</td>
</tr>
<tr>
<td>ABAJO, j</td>
