...
describe('l’écran des Réglages se monte', () => {
    ...
    test('un format verrouillé montre sa raison au lieu de disparaître', () => {
        ...
        const frozen = /** @type {HTMLElement} */ (container.querySelector('.phase-kind.frozen'));
    ...
    test('enregistrer en cours de tournoi passe par la liste de contrôle', async () => {
        ...
        await fireEvent.click(/** @type {HTMLElement} */ (container.querySelector('.actions button.primary')));
        expect(applied).toBe(0);
        const confirm = /** @type {HTMLElement} */ (container.querySelector('.confirm'));
...
describe('les têtes de série', () => {
    ...
    test('l’option est là, éteinte, et dit pourquoi', () => {
        ...
        const seeding = /** @type {HTMLInputElement} */ (boxes.find((b) => (b.closest('label')?.textContent || '').includes('Seeding')));
        expect(seeding).toBeTruthy();
        expect(seeding.checked).toBe(false);
        // La raison est dans l'infobulle, pas dans une leçon à l'écran.
        expect(/** @type {HTMLLabelElement} */ (seeding.closest('label')).getAttribute('title')).toContain('no protected seeds');
    });
